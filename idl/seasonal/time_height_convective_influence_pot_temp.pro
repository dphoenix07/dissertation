PRO TIME_HEIGHT_CONVECTIVE_INFLUENCE_POT_TEMP, event, start_date, end_date, $
	DOMAIN         = domain, $
...
;+
; Name:
;		TIME_HEIGHT_CONVECTIVE_INFLUENCE_POT_TEMP
; Purpose:
...
;		areas.
; Calling sequence:
;		TIME_HEIGHT_CONVECTIVE_INFLUENCE_POT_TEMP, run, start_date, end_date
; Example: TIME_HEIGHT_CONVECTIVE_INFLUENCE_POT_TEMP,'20110518','20110518T1300Z','20110527T1200Z', tracer = 'O3'
; Input:
...
;
;								2018-05-22. NOTE: It appears this program was never 
;											completed. An attempt to switch the vertical
;											coordinate to potential temperature was started
;											but abandoned and would need to be completed
;											before using. "time_height_convective_influence_new.pro"
;											works with altitude as the vertical coordinate.  
;-
 ...
 ;   z_trop   = CALC_TROP_MODE(z_trop, scheme, threshold) 												;Filter tropopause values
    ...
    FOR ii = 0, dim[0]-1 DO BEGIN
    	FOR jj = 0, dim[1]-1 DO BEGIN
    		...
    		isen_trop [ii,jj] = theta.values[ii,jj,kk]
    	ENDFOR
    ENDFOR
	...
	PRINT, 'Mean pot. temp. at tropopause = ' + STRING(MEAN(xyz_trop[itrop],/NAN))
