PRO OZONE_WRAPPING, event, scheme, start_date, end_date, $
	DOMAIN   = domain, $
	REGION   = region, $
	TROP_REL = trop_rel, $
	REFL	 = refl, $
...
;		OZONE_WRAPPING
; Purpose:
;		Find occurrences of ozone wrapping and estimate scale  
; Calling sequence:
...
;	tr		= (WRF_READ_VAR('ST_tracer'      , date, event, scheme, DOMAIN=domain, INDICES=region)).values*1.0E3
    ...
    tr		 [trp_conv] = tr	  [trp_conv]

    xyz_trop [tp_bad  ] = !Values.F_NaN
    z		 [tp_bad  ] = !Values.F_NaN
    tr		 [tp_bad  ] = !Values.F_NaN
 
;;+ DBP commented out to look at plots	
 	zc_arr  = [ ]
	cld_arr = [ ]
...
;; Find cloud depth
	...
	;;Find updraft threshold in fortran code and take that out;;	
...
;;+ DBP current ozone wrapping check
PRINT, 'start loop'	
	FOR i = 0, dim[0]-1 DO BEGIN
		FOR j = 0, dim[1]-1 DO BEGIN
			FOR k = 0, dim[2]-2 DO BEGIN
				IF ((conv[i,j,k] GT 0.1) AND (updraft[i,j,k] LT 1.0)) THEN BEGIN
					IF (FINITE(zc_arr[i,j,k]*cld_arr[i,j,k])) THEN BEGIN
						k_start = k
						FOR k1 = k_start, k_start+10 DO BEGIN
							cloud_depth = (zc_arr[i,j,k1]*cld_arr[i,j,k1]) - (zc_arr[i,j,k_start]*cld_arr[i,j,k_start])
							IF (cloud_depth GT 750.0) THEN $
								IF (utls_o3[i,j,k_start] GT 175.0) THEN BEGIN ;$
									PRINT, i, j, k_start, k1, zc_arr[i,j,k_start], cloud_depth, utls_o3[i,j,k_start]
									dum[i,j,k_start] = 999999.9
								ENDIF
						ENDFOR
						k = k1
					ENDIF
		...
		ENDFOR
	ENDFOR
time_loop1 = SYSTIME(/SECONDS)-time0
PRINT, 'loop1 =', time_loop1

	wrap1 = WHERE(dum EQ 999999.9)
	wrap_arr = [wrap_arr, wrap1]								
;;-end


;;+ DBP test to cut out loops
;wrap_arr = [ ]
;dum = FLTARR(dim[0],dim[1],dim[2])

;time0 = SYSTIME(/SECONDS)
;good = WHERE((conv GT 0.1) AND (updraft LT 0.1), count, COMPLEMENT=bad)
;zc_arr [bad] = !Values.F_NaN
;cld_arr[bad] = !Values.F_NaN
;
;PRINT, 'start loop'	
;FOR i = 0, dim[0]-1 DO BEGIN
;	FOR j = 0, dim[1]-1 DO BEGIN
