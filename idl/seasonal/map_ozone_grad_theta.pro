PRO MAP_OZONE_GRAD_THETA, event, scheme, start_date, end_date, $
	DOMAIN   = domain, $
	REGION   = region, $
	TRACER   = tracer, $
	THRES	 = thres, $
...
;    xyz_trop[filt] = !Values.F_NaN
	...
	o3_interp = FLTARR(dim[0],dim[1],N_ELEMENTS(z_new))
	theta_interp = FLTARR(dim[0],dim[1],N_ELEMENTS(z_new))
    ...
    FOR ii=0, dim[0]-1 DO BEGIN
    	FOR jj=0, dim[1]-1 DO BEGIN
    		z2[ii,jj,*] = INTERPOLATE(z[ii,jj,*],iz[ii,jj,*])
 			o3_interp[ii,jj,*] = INTERPOLATE(o3[ii,jj,*],iz[ii,jj,*])
    	ENDFOR
    ENDFOR		

;;+ Comment out if not using theta
;	;Interpolate theta array
;	FOR ii=0, dim[0]-1 DO BEGIN
;		FOR jj=0, dim[1]-1 DO BEGIN
; 			iz[ii,jj,*] = INTERPOL(FINDGEN(N_ELEMENTS(z[ii,jj,*])),z[ii,jj,*],z_new)
;		ENDFOR
...
;	FOR ii=0, dim[0]-1 DO BEGIN
;		FOR jj=0, dim[1]-1 DO BEGIN
;			theta_interp[ii,jj,*] = INTERPOLATE(theta[ii,jj,*],iz[ii,jj,*])
;		ENDFOR
;	ENDFOR	
;;-	

 	dim2   = SIZE(o3_interp,/DIMENSIONS)
 	map_o3 = FLTARR(dim2[0],dim2[1]) * !Values.F_NaN
	theta1 = FLTARR(dim2[0],dim2[1])
 	trop_arr = FLTARR(dim2[0],dim2[1])
 	
 	do3dz  = FLTARR(dim2[0],dim2[1],dim2[2]-2) 
 	 
	FOR ii = 0, dim2[0]-1 DO BEGIN
		FOR jj = 0, dim2[1]-1 DO BEGIN
			FOR kk = 1, dim2[2]-3 DO BEGIN
				IF (z2[ii,jj,kk] GT 5000.0) THEN BEGIN
					do3dz[ii,jj,kk] = (o3_interp[ii,jj,kk+1]-o3_interp[ii,jj,kk])/(z2[ii,jj,kk+1]-z2[ii,jj,kk])
				ENDIF ELSE BEGIN
					do3dz[ii,jj,kk] = 0.0
				ENDELSE
			ENDFOR
	...
	;Search vertically for first occurence of do3dz gt 60 ppbv
	FOR ii = 0, dim2[0]-1 DO BEGIN
		FOR jj = 0, dim2[1]-1 DO BEGIN
			FOR kk = 1, dim2[2]-3 DO BEGIN
				IF (do3dz[ii,jj,kk]*1.0E3 GE 60.0) THEN BEGIN
					IF (o3_interp[ii,jj,kk] GE 80.0) THEN BEGIN
						PRINT, ii,jj,'Criteria Met at k = ', kk
						PRINT, do3dz[ii,jj,kk]*1.0E3, o3_interp[ii,jj,kk], z2[ii,jj,kk], theta_interp[ii,jj,kk]
						theta1[ii,jj] = theta_interp[ii,jj,kk]
						trop_arr[ii,jj] = z2[ii,jj,kk]
						kk = dim2[2]
					ENDIF
				ENDIF
			ENDFOR
...
;	FOR ii = 0, dim[0]-1 DO BEGIN
;		FOR jj = 0, dim[1]-1 DO BEGIN
;			theta_thres = FLTARR(dim[2]) + (theta1[ii,jj] - thres)
;			index = VALUE_LOCATE(REFORM(theta[ii,jj,*],dim[2],1,1),theta_thres)
;			map_o3 [ii,jj] = o3_tr[ii,jj,index[0]]					
;			IF (index[0] EQ -1) THEN map_o3[ii,jj] = !Values.F_NaN
;		ENDFOR
;	ENDFOR
;;-

;;+ Not sure what this is, may be left over from older code (leave commented out)
;	FOR ii = 0, dim[0]-1 DO BEGIN
;		FOR jj = 0, dim[1]-1 DO BEGIN
;			index1 = VALUE_LOCATE(REFORM(z[ii,jj,*],dim[2],1,1),REFORM(xyz_trop[ii,jj,*],dim[2],1,1))
;			theta_thres = FLTARR(dim[2]) + (theta[ii,jj,index1[0]] - thres)
;			index = VALUE_LOCATE(REFORM(theta[ii,jj,*],dim[2],1,1),theta_thres)
;			map_o3 [ii,jj] = o3_tr[ii,jj,index[0]]
...
;	 map_bar_title = 'Tracer = ' + STRING(tracer) + ', Threshold (K) = '	+ STRING(thres)												;Set color bar title
	 map_bar_title = 'Potential Temperature at dO3/dz = 60 ppb/km'
	 map_bar_min   = 320.0																						;Set echo top minimum
	 map_bar_max   = 380.0																					;Set echo top maximum
