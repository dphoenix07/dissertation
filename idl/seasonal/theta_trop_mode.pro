PRO THETA_TROP_MODE, event, scheme, start_date, end_date, $
	DOMAIN   = domain, $
	REGION   = region, $
	HIST	 = hist, $
	PNG	     = png, $
...
;		THETA_TROP_MODE
; Purpose:
;		Compute the mode of the theta value at the tropopause at each analysis time. 
; Calling sequence:
;		THETA_TROP_MODE, run, scheme, start_date, end_date
; Input:
...
;								Using the mode of the theta value at the tropopause over-
;								estimates the theta value at some latitudes and under-
;								estimates it at others. Would be better to break the 
;								domain into smaller sections and find the mode in each 
;								section (the typical theta value at the tropopause varies
;								across the domain so one value is not representative of
;								all values).
;-
...
FOREACH date, date_arr DO BEGIN
    ...
    o3      = (WRF_READ_VAR('O3'             , date, event, scheme, DOMAIN=domain, INDICES=region)).values*1.0E3					
 	utls	= (WRF_READ_VAR('UTLS_tracer'    , date, event, scheme, DOMAIN=domain, INDICES=region)).values*1.0E3
    ...
    xyz_trop[filt] = !Values.F_NaN
 	...
 	trop_theta = FLTARR(dim[0],dim[1]) * !Values.F_NaN
 	 
 	check = 0
	FOR ii = 0, dim[0]-1 DO BEGIN
		FOR jj = 0, dim[1]-1 DO BEGIN
			FOR kk = 0, dim[2]-1 DO BEGIN
				IF (ABS(z[ii,jj,kk] - ztrop[ii,jj]) LT 1.0) THEN BEGIN
;					PRINT, ii, jj, kk, ' Tropopause identified = ', z[ii,jj,kk], ztrop[ii,jj]
					trop_theta [ii,jj] = theta[ii,jj,kk]
				ENDIF
			ENDFOR
		ENDFOR
	ENDFOR
	
	good = WHERE(FINITE(trop_theta))
	hist    = HISTOGRAM(trop_theta[good], MIN = MIN(trop_theta,/NAN))						 						;Calculate density
	bigfreq = MAX(hist,/NAN)																			;Find most largest density value
	mode    = WHERE(hist EQ bigfreq) + MIN(trop_theta,/NAN)											;Find tropopause mode
	
	PRINT, mode

	mode = REBIN(mode,dim[0],dim[1],dim[2]) - 5.0
	trop = WHERE(theta LT mode, count)
	
	wrap = WHERE(utls[trop] GT 100.0, wr_count)
	
;	PRINT, utls[[trop[wrap]]]
...
IF (wr_count GT 0) THEN BEGIN
	FOR i=0, wr_count-1 DO BEGIN
		PLOTS, (xx)[trop[wrap]], (yy)[trop[wrap]], $																		;Overplot plane symbol
			PSYM    = 8, $
