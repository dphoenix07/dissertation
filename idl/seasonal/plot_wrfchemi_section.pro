PRO PLOT_WRFCHEMI_SECTION, experiment1, state1, x0, y0, x1, y1, date1, $
	DOMAIN       = domain, $
	MAP_TYPE     = map_type, $
...
IF (N_ELEMENTS(section_type) EQ 0) THEN section_type = 2
IF (N_ELEMENTS(chemical    ) EQ 0) THEN chemical     = 'E_CO'
...
;Read in dummy wrf output file to get x,y coordinates
date = MAKE_DATE(2013,8,5,12)
experiment = '20130805'
state 	   = 'nest'

x     = WRF_READ_VAR('Longitude',       date, experiment, state, DOMAIN = 1)			;Read variables
y     = WRF_READ_VAR('Latitude',        date, experiment, state, DOMAIN = 1)
z     = WRF_READ_VAR('Z',               date, experiment, state, DOMAIN = 1)
cloud = WRF_READ_VAR('CLOUD_MIX_TOTAL', date, experiment, state, DOMAIN = 1)
trop  = WRF_READ_VAR('Z_trop',          date, experiment, state, DOMAIN = 1)
...
; 		 map_plot = MEDIAN(map_plot1,25)
		 ...
		 END
	2 : BEGIN    
    	var           = WRFCHEMI_READ_VAR(chemical, date1, experiment1, state1, DOMAIN = domain)		;Read tracer
 		map_plot	  = (var.values[*,*,0]*factor) 
    	map_bar_title = STRING(chemical)																;Set color bar title
    	map_bar_min   = 0.0																			;Set tracer minimum
    	map_bar_max   = 50.0																			;Set tracer maximum
...
CASE section_type OF
	...
	2 : BEGIN
		...
		var = WRFCHEMI_READ_VAR(chemical, date1, experiment1, state1, DOMAIN = domain)					;Read trace gas from WRF output
	    fill_sect = INTERPOLATE(var.values,   iisect, jjsect, kksect, MISSING = !Values.F_NaN)	;Interpolate trace gas concentration to color-fill cross-section
		 ...
		 CASE chemical OF
		 	'E_CO' : BEGIN 
					 ...
					 sect_bar_ticks   = 4
					 sect_bar_title   = 'Carbon Monoxide Emissions (mol km^-1 hr^-1)'
					 END

		 	'E_ISO' : BEGIN 
					 fill_sect = fill_sect * factor
					 fill_sect_table  = HCL_COLOR_TABLE(20, HUE_RANGE = [100.0, 300.0])			;Set color-fill cross-section specs
					 sect_bar_min     = 0.0	
					 sect_bar_max     = 50.0
					 fill_sect_levels = ((sect_bar_max-sect_bar_min)/20.0)*FINDGEN(20)
					 sect_bar_ticks   = 4
					 sect_bar_title   = 'Isoprene Emissions (mol km^-1 hr^-1)'
					 END
		 	...
		 	'E_HCHO' : BEGIN 
					 ...
					 fill_sect_levels = ((sect_bar_max-sect_bar_min)/20.0)*FINDGEN(20)
					 sect_bar_ticks   = 4
					 sect_bar_title   = 'Formaldehyde Emissions (mol km^-1 hr^-1)'
					 END
		 	...
		 	'E_NO' : BEGIN 
					 ...
					 fill_sect_levels = ((sect_bar_max-sect_bar_min)/20.0)*FINDGEN(20)
					 sect_bar_ticks   = 4
					 sect_bar_title   = 'NO Emissions (mol km^-1 hr^-1)'
					 END
		 	...
		 	'E_SO2' : BEGIN 
					 ...
					 fill_sect_levels = ((sect_bar_max-sect_bar_min)/20.0)*FINDGEN(20)
					 sect_bar_ticks   = 4
					 sect_bar_title   = 'SO2 Emissions (mol km^-1 hr^-1)'
					 END
