PRO MAP_OZONE_CLOUDTOP, event, scheme, start_date, end_date, $
	DOMAIN   = domain, $
	REGION   = region, $
	TRACER   = tracer, $
	THRES	 = thres, $
...
;		MAP_OZONE_CLOUDTOP
; Purpose:
;		Map ozone tracer concentrations on at an altitude relative to the cloud top.
;		Calculates cloud top in a column and creates a 2D matrix of that value (to 
;		create an smoothed anvil). Plots O3 concentration relative to that altitude.
; Calling sequence:
;		MAP_OZONE_CLOUDTOP, run, scheme, start_date, end_date
; Input:
...
; Output:
;		2D contour map of ozone tracer concentrations relative to anvil cloud top altitude
; Keywords:
;		DOMAIN  : Optional keyword to specify output domain name. Default is 'd01'.
;		TRACER  : Ozone tracer to use (UTLS_tracer is default).
; Author and history:
;		Daniel B. Phoenix	    2018-01-26. 
;								
;-
...
outdir  = !WRF_DIRECTORY + event + '/' + scheme + '/plots/ozone_cloudtop/'
FILE_MKDIR, outdir
...
; 	cloud_top = ((MAX((cloud.values GE 1.0E-5)*z, DIM = 3, /NAN)))
 	cloud_top = ((MAX((cloud.values GE 5.0E-4)*z, DIM = 3, /NAN)))
	cloud_top1 = REBIN(cloud_top,dim[0],dim[1],dim[2],/SAMPLE)

	FOR ii = 0, dim[0]-1 DO BEGIN
		FOR jj = 0, dim[1]-1 DO BEGIN
			index_cloud = VALUE_LOCATE(REFORM(z[ii,jj,*],dim[2],1,1),REFORM(cloud_top1[ii,jj,*],dim[2],1,1))
			index_trop  = VALUE_LOCATE(REFORM(z[ii,jj,*],dim[2],1,1),REFORM(xyz_trop[ii,jj,*],dim[2],1,1))
			IF (index_cloud[0] GT thres) THEN map_o3[ii,jj] = MEAN(o3[ii,jj,index_cloud[0]-thres:index_cloud[0]])
			IF (index_cloud[0] EQ -1) THEN map_o3[ii,jj] = MEAN(o3[ii,jj,index_trop[0]-thres:index_trop[0]])
		ENDFOR
	ENDFOR
...
ENDIF ELSE $	
	...
	ENDIF
	 ...
	 map_bar_max   = 400.0																					;Set echo top maximum
     ...
     map_levels    = [-100.0,((map_bar_max-map_bar_min)/20.0)*FINDGEN(20)]								;Set contour levels
