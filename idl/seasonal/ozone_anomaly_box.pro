PRO OZONE_ANOMALY_BOX, event, scheme, start_date, end_date, $
	DOMAIN     = domain, $
	REGION     = region, $
	TROP_REL   = trop_rel, $
	REFL	   = refl, $
...
;		OZONE_ANOMALY_BOX
; Purpose:
;		Find where O3 concentration at point exceeds the median O3 concentration of some 
;		user specified box.
; Calling sequence:
;		OZONE_ANOMALY_BOX, run, scheme, start_date, end_date
; Input:
...
;		TROP_REL   : Plots cloud tops relative to the tropopause (DO NOT SET UNLESS MAP OF TROP-REL CLOUD TOPS DESIRED)
;		WRITE_FILE : Writes data to file to plot later, if turned off will produce maps.
...
outdir  = !WRF_DIRECTORY + event + '/' + scheme + '/reduced/o3_wrapping/'
...
FOREACH date, date_arr DO BEGIN
    ...
    IF (scheme NE 'd03_30km') THEN BEGIN
    ...
    ENDIF					
    IF (scheme EQ 'd03_30km') THEN BEGIN
        tr      = (WRF_READ_VAR('O3_tracer'   	 , date, event, scheme, DOMAIN=domain, INDICES=region)).values*1.0E3
...
;    ztrop    = CALC_TROP_MODE(ztrop, scheme, threshold) 												;Filter tropopause values
    ...
    xyz_trop[filt] = !Values.F_NaN
 	...
 	;Define box size (value will be for 2 dimensions)
	box = 31
	
	iz    = FLTARR(dim[0],dim[1])
	depth = FLTARR(dim[0]*dim[1],dim[2])
	
	FOR kk = 20, 70 DO BEGIN
		PRINT, kk
		mean_o3 = MEDIAN(o3[*,*,kk], box) 													;Average ozone over specified area
		anom_o3 = o3[*,*,kk] - mean_o3														;Compute ozone perturbations from area average
		anom_o3 = REFORM(anom_o3, dim[0]*dim[1])
		
        z1 = REFORM(z,dim[0]*dim[1],dim[2])													
        z2 = z1[*,1:99] 
        dz = z2-z1																			
        
		iflag = WHERE(anom_o3 GT 30, count, COMPLEMENT = nflag)								;Flag boxes where ozone anomaly is larger than 30 ppb
		IF (N_ELEMENTS(iflag) GT 1) THEN BEGIN
			z_lev = dz[*,kk]																;Calculate depth of wrapping
			iz [iflag] = z_lev[iflag]														
			iz [nflag] = 0.0
	...
	depth1 = REFORM(depth,dim[0],dim[1],dim[2])
	map_plot1 = TOTAL(depth1,3)
	map_plot2 = map_plot1*0.001
   ...
   ; map_plot2 = map_plot2*1.0E-3 
	...
	IF (KEYWORD_SET(write_file)) THEN BEGIN
	    ...
	    vid = NCDF_VARDEF(oid, 'Wrapping_Points', [xid, yid, zid], /FLOAT)												;Define the latitude variable
	    NCDF_ATTPUT, oid, 'Wrapping_Points', 'long_name', 'Anvil Wrapping Points'									;Name attribute
	    NCDF_ATTPUT, oid, 'Wrapping_Points', 'units',     'km'												;Units attribute
	    ...
	    vid = NCDF_VARDEF(oid, 'Reflectivity', [xid, yid, zid], /FLOAT)												;Define the latitude variable
	    NCDF_ATTPUT, oid, 'Reflectivity', 'long_name', '10-cm Reflectivity'									;Name attribute
	    NCDF_ATTPUT, oid, 'Reflectivity', 'units',     'dBZ'												;Units attribute
	    ...
	    NCDF_VARPUT, oid, 'Wrapping_Points' , depth1
	    
	    NCDF_VARPUT, oid, 'Wrapping_Depth', map_plot2      
	    
	    NCDF_VARPUT, oid, 'Reflectivity', R.values																		;Write reflectivity
