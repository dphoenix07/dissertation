...
IF (N_ELEMENTS(var		 ) EQ 0) THEN var		 = 'T'
...
FOR s = 0, nt DO BEGIN
	FOR i = 0, 2 DO BEGIN
		x    = WRF_READ_VAR('Longitude', TIME_INC(start_date,s*dt), event, scheme[i], DOMAIN = domain, INDICES = region)				;Read variables
		y    = WRF_READ_VAR('Latitude' , TIME_INC(start_date,s*dt), event, scheme[i], DOMAIN = domain, INDICES = region)
		z    = (WRF_READ_VAR('Z'	   , TIME_INC(start_date,s*dt), event, scheme[i], DOMAIN = domain, INDICES = region)).values
		hgt  = (WRF_READ_VAR('Z0'	   , TIME_INC(start_date,s*dt), event, scheme[i], DOMAIN = domain, INDICES = region)).values
		pblh = (WRF_READ_VAR('PBL'	   , TIME_INC(start_date,s*dt), event, scheme[i], DOMAIN = domain, INDICES = region)).values
		u    = (WRF_READ_VAR('u' 	   , TIME_INC(start_date,s*dt), event, scheme[i], DOMAIN = domain, INDICES = region)).values
		v    = (WRF_READ_VAR('v'       , TIME_INC(start_date,s*dt), event, scheme[i], DOMAIN = domain, INDICES = region)).values
		dim  = SIZE(u, /DIMENSION)
		wind = SQRT(u^2 + v^2)																						;Horizontal wind
		...
		CASE var OF
			"o3"  : BEGIN 
				plt_var  = (WRF_READ_VAR('O3'  	, TIME_INC(start_date,s*dt), event, scheme[i], DOMAIN = domain, INDICES = region)).values * 1.0E3
				xtitle   = 'Ozone Concentration (ppb)'
				xrange   = [45, 70]
			END

			"co"  : BEGIN 
				plt_var  = (WRF_READ_VAR('CO'  	, TIME_INC(start_date,s*dt), event, scheme[i], DOMAIN = domain, INDICES = region)).values * 1.0E3
			...
			"nox" : BEGIN
				plt_var  = (WRF_READ_VAR('NO' 	, TIME_INC(start_date,s*dt), event, scheme[i], DOMAIN = domain, INDICES = region)).values * 1.0E6
				plt_var += (WRF_READ_VAR('NO2' 	, TIME_INC(start_date,s*dt), event, scheme[i], DOMAIN = domain, INDICES = region)).values * 1.0E6 
				xtitle   = 'NOx Concentration (ppt)'
				xrange   = [0, 1000]
			END
			...
			"theta" : BEGIN
				plt_var = (WRF_READ_VAR('T', TIME_INC(start_date,s*dt), event, scheme[i], DOMAIN = domain, INDICES = region)).values						;Read temperature variable from WRF output
		 		plt_var = ((1000.0/(WRF_READ_VAR('P', TIME_INC(start_date,s*dt), event, scheme[i], DOMAIN = domain, $
			...
			"h2o"	: BEGIN
				plt_var  = (WRF_READ_VAR('H2O'  , TIME_INC(start_date,s*dt), event, scheme[i], DOMAIN = domain, INDICES = region)).values * 1.0E3
				xtitle   = 'Water Vapor (g kg-1)'
				xrange   = [0, 12]
			END

			"T"	: BEGIN
				plt_var  = (WRF_READ_VAR('T'  , TIME_INC(start_date,s*dt), event, scheme[i], DOMAIN = domain, INDICES = region)).values 
	...
	;;			xrange   = [273, 310]
			...
			"Td"	: BEGIN
				plt_var  = (WRF_READ_VAR('Td'  , TIME_INC(start_date,s*dt), event, scheme[i], DOMAIN = domain, INDICES = region)).values
...
;			POSITION  = t1_pos, $
	...
	t2 = TEXT(515,415,  'MOR', FONT_SIZE=12, /DEVICE, FONT_COLOR = !COLOR.blue)
	t3 = TEXT(515,390,  'NSSL',FONT_SIZE=12, /DEVICE, FONT_COLOR = !COLOR.red)
	t4 = TEXT(515,365,  'MY',  FONT_SIZE=12, /DEVICE, FONT_COLOR = !COLOR.cyan)
