PRO MAP_WRF_OZONE, date, run, experiment, $
	OFFSET = offset, $
	DOMAIN = domain, $
	REGION = region, $
	Z_buff = z_buff, $
...
; Calling sequence:
;		MAP_WRF_OZONE, run, experiment, date
; Input:
;		run   : String variable of run name. (e.g., 'milbyau')
;		experiment : String variable of initial state. (e.g., 'wrfout')
;		date  : Desired date {CDATE}.
; Output:
;		A map of simulated ozone at a specified model level. 
; Keywords:
...
o  = WRF_READ_VAR('O3',    date, run, experiment, DOMAIN = domain, INDICES = region)

o.values = o.values * 1.0E3 
...
IF KEYWORD_SET(image) THEN BEGIN
	...
	image0 = (o.values[*,*,50])[offset:(dim[0]-(1+offset)), offset:(dim[1]-(1+offset))]
	dim    = dim - (2*offset)
...
ENDIF ELSE $	
	CONTOUR, (o.values[*,*,50]), x.values, y.values, $												;Contour reflectivity values
		OVERPLOT  = 1, $
		FILL      = 1, $
		LEVELS    = rlevels, $
		C_COLOR   = table
