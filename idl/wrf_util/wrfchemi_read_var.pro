FUNCTION WRFCHEMI_READ_VAR, var, date, run, state, $
	DOMAIN  = domain, $
	INDICES = indices
...
; Calling sequence:
;		T = WRFCHEMI_READ_VAR('T', MAKE_DATE(2012, 4, 5), '20120519', 'morrison')
; Inputs:
...
infile      = !WRF_DIRECTORY + run + '/' + state + '/reduced_wrfchemi/' + dom_string $
