PRO MAP_THETA_GRAD, date, run, experiment, $
	LEV          = lev, $
	PLT_NOX      = plt_nox, $
	PLT_CO		 = plt_co, $
	OFFSET       = offset, $
...
theta =  WRF_READ_VAR('T'	     , date, run, experiment, DOMAIN = domain)							;Read temperature variable from WRF output
...
dth_dz = FLTARR(dim[0],dim[1],dim[2]-1)*!Values.F_NaN
FOR ii=0, dim[0]-1 DO BEGIN
	FOR jj=0,dim[1]-1 DO BEGIN
		FOR kk = 0, dim[2]-2 DO BEGIN
			dth_dz[ii,jj,kk] = (theta.values[ii,jj,kk+1] - theta.values[ii,jj,kk-1]) / (z[ii,jj,kk+1] - z[ii,jj,kk-1])
		ENDFOR
	ENDFOR
ENDFOR
...
ENDIF ELSE IF KEYWORD_SET(plt_nox) THEN BEGIN
	...
	key 	= 'sfc_nox'
ENDIF ELSE BEGIN
	table   = BLUE_RED_24(60)
	rlevels = 0.001 * FINDGEN(N_ELEMENTS(table)) - 0.03															;Set reflectivity contour levels
	key     = 'dth_dz'
ENDELSE
...
outdir  = !WRF_DIRECTORY + run + '/' + 'paper/plots/theta_grad_icloud_maps/' 
...
CONTOUR, dth_dz[*,*,k], x.values, y.values, $													
	OVERPLOT  = 1, $
	FILL      = 1, $
	LEVELS    = rlevels, $
	C_COLOR   = table
...
;VELOVECT, ureg, vreg, xreg, yreg, OVERPLOT  = 1, LENGTH = 6.0
