PRO PLOT_WRF_CROSS_SECTION, var, date, run, state, $
	TROPW   = tropw, $
	TRACER  = tracer, $
	REGION  = region, $
	INDICES = indices, $
...
cloud = WRF_READ_VAR('CLOUD_MIX_TOTAL', date, run, state, DOMAIN = domain, INDICES = rindices)
...
theta = WRF_READ_VAR('T',         		date, run, state, DOMAIN = domain, INDICES = rindices)
u_wind = WRF_READ_VAR('u', 	 	  		date, run, state, DOMAIN = domain, INDICES = rindices)
v_wind = WRF_READ_VAR('v', 	 	  		date, run, state, DOMAIN = domain, INDICES = rindices)
w_wind = WRF_READ_VAR('w', 	 	 		date, run, state, DOMAIN = domain, INDICES = rindices)

cloud.values = cloud.values * 1.0E9

wind   = SQRT(u_wind.values^2 + v_wind.values^2 + w_wind.values^2)
z_trop = trop.values
trop   = CALC_TROP_MODE(z_trop, state, 2000.0)

refl  = WRF_READ_VAR('REFL'		, date, run, state, DOMAIN = domain)
lon   = WRF_READ_VAR('Longitude', date, run, state, DOMAIN = domain)
lat   = WRF_READ_VAR('Latitude',  date, run, state, DOMAIN = domain)

IF (state EQ 'nssl') THEN BEGIN
	bad = WHERE (refl.values EQ 0.0, bad_count, COMPLEMENT = good, NCOMPLEMENT = good_count)	
	refl.values [bad ] = -35.0000
	refl.values [good] = refl.values [good]
	IF (map_var EQ 'REFL') THEN BEGIN
		bad = WHERE (vmap.values EQ 0.0, bad_count, COMPLEMENT = good, NCOMPLEMENT = good_count)
		vmap.values [bad ] = -35.0000
		vmap.values [good] = vmap.values [good]	
	ENDIF
ENDIF
...
CASE var OF
	'REFL'     : BEGIN
					 ...
					 levels = [-100.0, 5.0 + 5.0*FINDGEN(N_ELEMENTS(table))]														;Set reflectivity contour levels
					 range  = [5, 80]
					 ticks  = 5
...
mlevels  = [-100.0, 5.0 + 5.0*FINDGEN(N_ELEMENTS(mtable))]
mrange   = [5, 80]
mticks   = 5
mtitle   = 'Composite Reflectivity (dBZ)'
map_plot = MAX(refl.values, DIM = 3, /NAN)
;map_plot = refl.values[*,*,37]
