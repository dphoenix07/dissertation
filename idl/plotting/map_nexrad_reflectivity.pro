...
;		limit = [33.1, 258.3, 39.0, 263.9] 
		limit = [33.5, 257.6, 38.5, 265.5]													; Matches model domain
;		limit = [33.5, 258.3, 38.5, 264.5]													; Better aspect ratio
;		limit = [35.0, 259.6, 37.0, 262.0]	
		experiment = 'nssl_ysu'								
	END
...
;		limit = [30.8, 256.9, 44.7, 276.5]
		limit = [27.8, 254.9, 46.7, 288.0]
		nexrad  = NEXRAD_3_1_LEVEL3_3D_READ_NCDF(date, version = '3_1', product = '3d')
		experiment = 'seasonal_final/bigger_domain'
		domain = 1
	END
	
	'20130805' : BEGIN
		limit = [20.8, 243.9, 43.7, 283.5]
		nexrad  = NEXRAD_3_1_LEVEL3_3D_READ_NCDF(date, version = '3_1', product = '3d')
		experiment = 'nest_final'
		domain = 1
	END
ENDCASE

x     = WRF_READ_VAR('Longitude', date, event, experiment, DOMAIN = domain)		;Read variables
y     = WRF_READ_VAR('Latitude',  date, event, experiment, DOMAIN = domain)

dim   = SIZE(x.values,/DIMENSIONS)
...
;table   = [COLOR_24(200, 200, 200),GRIDRAD_VIEWER_COLOR(1)]												;Set reflectivity color table
title0   = 'Column-Maximum Map'

dim = SIZE(data.dbz.values, /DIMENSIONS)
wfactor = 400.0/(dim[0]) + 400.0/(dim[1])

outdir  = !WRF_DIRECTORY + event + '/paper/plots/nex_refl/'
...
;MAP_SET, 0, 180, 0, CONIC = 1, STANDARD_PARALLELS=[0,1], /NOBORDER, $																;Draw map
