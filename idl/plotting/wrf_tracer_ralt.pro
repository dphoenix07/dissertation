PRO WRF_TRACER_RALT, tracer, ralt, cloud, $
	TITLE      = title, $
	XTITLE     = xtitle, $
	XRANGE     = xrange, $
	XLOG       = xlog, $
...
;		 
; Calling sequence:
;		WRF_TRACER_RALT, tracer, ralt, refl, cloud
; Input:
...
IF (N_ELEMENTS(filtering) EQ 0) THEN filtering = 0
...
bin     = LONG((tracer-xrange[0])/dx) + nxbin*(LONG((ralt-yrange[0])/dy))						;bin concentrations

hist = HISTOGRAM(bin, BINSIZE = 1, MIN = 0, MAX = (nxbin*nybin -1))								;Calculate density
hist = REFORM(hist, nxbin, nybin)																;Rebinning in x-y space
