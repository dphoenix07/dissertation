...
x  = WRF_READ_VAR('Longitude', date1, run, 'nssl_ysu', DOMAIN = domain, INDICES = region)		;Read variables
y  = WRF_READ_VAR('Latitude',  date1, run, 'nssl_ysu', DOMAIN = domain, INDICES = region)
...
;	date_string = MAKE_ISO_DATE_STRING(date, PREC='MINUTE', /COMPACT, /UTC)							;Create date string
	...
	outdir  = !WRF_DIRECTORY + run + '/tmatrix/' + experiment + '/plots/refl/'
