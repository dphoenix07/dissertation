...
;	FOREACH d, date_arr DO PLOT_WRF_SECTION_AVERAGE, STRING(event), STRING(scheme),325,700,700,400, d, dom=1,map_type=2,section_type=2,chemical='w',KEY=key, zrange=[8,13], PNG=1
	...
	FOREACH d, date_arr DO PLOT_WRF_SECTION_DBP, STRING(event), STRING(scheme),350,710,800,520, d, dom=1,map_type=2,section_type=2,chemical='w',PNG=1

ENDIF

IF KEYWORD_SET(section_wind) THEN BEGIN
	key = 'hwind'
	FOREACH d, date_arr DO PLOT_WRF_SECTION_DBP, STRING(event), STRING(scheme),75,700,2019,100, d, dom=1,map_type=2,section_type=3,chemical='O3',zrange=[7,15],KEY=key, PNG=1
...
;plot_wrf_section_dbp,'20120530_ncar','d02_30km',135,260,535,130,make_date(2012,5,31,0),map_type=2,section_type=2,chemical='O3',dom=1 ,zrange=[7,18]
ENDIF
