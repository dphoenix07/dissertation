PRO OVERSHOOT_COUNTS, event, scheme, start_date, end_date, $
	DOMAIN   = domain, $
	REGION   = region, $
	TROP_REL = trop_rel, $
	ECHO_TOP = echo_top, $
...
; Calling sequence:
;		OVERSHOOT_COUNTS, run, scheme, start_date, end_date
; Input:
...
;    tr    = (WRF_READ_VAR('UTLS_tracer'    , date, event, scheme, DOMAIN=domain, INDICES=region)).values*1.0E3
	...
	IF (KEYWORD_SET(ECHO_TOP)) THEN BEGIN
		dbz_thres = 5.0
		overshoot = 0.001*((MAX((R.values GE dbz_thres)*z, DIM = 3, /NAN)) - xyz_trop[*,*,50])	
	ENDIF
	PRINT, MAX(overshoot,/NAN)
 
   FOR i=0,dim[0]-1 DO BEGIN
   	FOR j=0,dim[1]-1 DO BEGIN
   		IF (overshoot[i,j] GT -0.5) THEN over_count [i,j] = over_count[i,j]+1
   	ENDFOR
   ENDFOR
...
ENDIF ELSE $	
	 ...
	 map_bar_max   = 7																					;Set echo top maximum
...
epsfile = outdir +  'wrf_echo_' + date1  + '.eps'						;EPS filename
pdffile = outdir +  'wrf_echo_' + date1  + '.pdf'						;PDF filename
pngfile = outdir +  'wrf_echo_' + date1  + '.png'						;PNG filename
