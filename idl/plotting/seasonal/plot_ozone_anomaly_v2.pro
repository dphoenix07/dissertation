PRO PLOT_OZONE_ANOMALY_V2, event, scheme, start_date, end_date, $
	DOMAIN     = domain, $
	REGION     = region, $
	TROP_REL   = trop_rel, $
	REFL	   = refl, $
...
outdir  = !WRF_DIRECTORY + event + '/' + scheme + '/plots/utls_tracer_box3/stt/'
...
FOREACH date, date_arr DO BEGIN
   ...
   R 		  = (WRF_READ_VAR('REFL'        , date, event, scheme, DOMAIN=domain, INDICES=region))
    ...
    ENDIF ELSE $	
    	 ...
    	 map_table 	   = GRAYSCALE_24(40, 0.95, 0.0, PS = ps)																		;Color table for plotting 
		...
		rlevels = [1.0+5.0*FINDGEN(N_ELEMENTS(table))]													;Set reflectivity contour levels
   		...
   		z1 = mean(z,dim=1,/nan)
   		z2 = mean(z1,dim=1,/nan)
   		index5km = VALUE_LOCATE(z2, 5000.0)
   		
    	;CONTOUR, MAX(R.values, DIM=3), x, y, $															;Contour reflectivity values
    	CONTOUR, R.values[*,*,index5km], x, y, $
    		OVERPLOT  = 1, $
    		FILL      = 1, $
    		LEVELS    = rlevels, $
    		C_COLOR   = table, $
