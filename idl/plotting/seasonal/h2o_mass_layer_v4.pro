PRO H2O_MASS_LAYER_V4, run, experiment, start_date, end_date, $
	DOMAIN        = domain, $
	REGION        = region, $
...
; Calling sequence:
;		H2O_MASS_LAYER_V4, run, scheme, start_date, end_date
;		H2O_MASS_LAYER_V4, '20120530_ncar','d03_30km','20120530T2200Z','20120531T0500Z'
; Input:
...
IF (run EQ '20110518') THEN PRINT, 'midlats = ', midlats
IF (run EQ '20130805') THEN PRINT, 'tropics = ', tropics
...
FOREACH date, date_arr DO BEGIN
	...
	count = 0
	 ...
	 IF (KEYWORD_SET(mid_lat)) THEN igood = WHERE(trop_3d LT 14000.0, count, COMPLEMENT = ibad)

	IF (count GT 0) THEN h2o[ibad] = 0.0
	...
	h2o_kg_2d_total    = FLTARR(dim[0],dim[1])*!Values.F_NaN
	h2o_conv_2d_total  = FLTARR(dim[0],dim[1])*!Values.F_NaN
	h2o_nconv_2d_total = FLTARR(dim[0],dim[1])*!Values.F_NaN

	inan    = FLTARR(dim[0],dim[1])*!Values.F_NaN
	inan_c  = FLTARR(dim[0],dim[1])*!Values.F_NaN
	inan_nc = FLTARR(dim[0],dim[1])*!Values.F_NaN
	...
	FOR ii = 0, dim[0]-1 DO BEGIN
		...
		FOR jj = 0, dim[1]-1 DO BEGIN
			...
			cloudlocs = WHERE(cloud[ii,jj,index0:index] GT 1.0E-5)
			cloud_fraction[ii,jj] = FLOAT(N_ELEMENTS(cloudlocs))/FLOAT((N_ELEMENTS(cloud[ii,jj,index0:index])))
		
			IF (ii EQ 	     5) THEN flux_h2o_left [*,jj,*] = MEAN(u[5,		  jj,index0:index]*(h2o_kg[index0:index]))
			IF (ii EQ dim[0]-6) THEN flux_h2o_right[*,jj,*] = MEAN(u[dim[0]-6,jj,index0:index]*(h2o_kg[index0:index]))
			IF (jj EQ 	     5) THEN flux_h2o_bottom[ii,*,*]= MEAN(v[ii,	   5,index0:index]*(h2o_kg[index0:index]))
			IF (jj EQ dim[1]-6) THEN flux_h2o_top[ii,*,*]   = MEAN(v[ii,dim[1]-6,index0:index]*(h2o_kg[index0:index]))
			...
			h2o_kg_2d_total   [ii,jj] = TOTAL(h2o_kg   [ii,jj,index0:index],/NAN)
			h2o_conv_2d_total [ii,jj] = TOTAL(h2o_conv [ii,jj,index0:index],/NAN)
			h2o_nconv_2d_total[ii,jj] = TOTAL(h2o_nconv[ii,jj,index0:index],/NAN)

			inan    [ii,jj]= WHERE(FINITE(h2o	   [ii,jj,index0:index],/NAN),nan_count, COMPLEMENT = ivalue, NCOMPLEMENT = value_count)
			inan_c  [ii,jj]= WHERE(FINITE(h2o_conv [ii,jj,index0:index],/NAN),nan_count, COMPLEMENT = ivalue, NCOMPLEMENT = value_count_conv)
			inan_nc [ii,jj]= WHERE(FINITE(h2o_nconv[ii,jj,index0:index],/NAN),nan_count, COMPLEMENT = ivalue, NCOMPLEMENT = value_count_nconv)
		ENDFOR
	ENDFOR
	...
	time_layer_flux = [[[time_layer_flux]],[[flux_tot]]]
	...
	PRINT,'non-convective count dracula= ', value_count_nconv
  	...
  	h2o_wrap   = h2o_kg_2d/value_count
  	h2o_total  = h2o_kg_2d 
	
	h2o_ave_conv  = h2o_conv_2d /value_count_conv
	h2o_ave_nconv = h2o_nconv_2d/value_count_nconv
	h2o_tot_conv  = h2o_conv_2d 
	h2o_tot_nconv = h2o_nconv_2d	
...
IF (run EQ '20110518') THEN BEGIN 
    ...
    XYOUTS, 12, 6.80E4, '- dh2o/dt' , COLOR = COLOR_24('red'	  ), CHARSIZE=2, /DATA
    XYOUTS, 12, 6.70E4, '- net' 	, COLOR = COLOR_24('darkgreen'), CHARSIZE=2, /DATA
    XYOUTS, 12, 6.60E4, '- h2o flux', COLOR = COLOR_24('blue'     ), CHARSIZE=2, /DATA
    XYOUTS, 12, 6.50E4, '- Tot h2o' , COLOR = COLOR_24('gray60'   ), CHARSIZE=2, /DATA
    XYOUTS, 12, 6.40E4, 'h2o-conv'  , COLOR = COLOR_24('gray30'   ), CHARSIZE=2, /DATA
    XYOUTS, 12, 6.30E4, 'h2o-nconv' , COLOR = COLOR_24('gray70'   ), CHARSIZE=2, /DATA
ENDIF
...
;    	YRANGE   = [2.0E4,3.0E4], $
    ...
    XYOUTS, 84 , 1.08E4, '- dh2o/dt' , COLOR = COLOR_24('red'	  ), CHARSIZE=2, /DATA
    XYOUTS, 84 , 1.07E4, '- net' 	, COLOR = COLOR_24('darkgreen'), CHARSIZE=2, /DATA
    XYOUTS, 84 , 1.06E4, '- h2o flux', COLOR = COLOR_24('blue'     ), CHARSIZE=2, /DATA
    XYOUTS, 120, 1.08E4, '- Tot h2o' , COLOR = COLOR_24('gray60'   ), CHARSIZE=2, /DATA
    XYOUTS, 120, 1.07E4, 'h2o-conv'  , COLOR = COLOR_24('gray30'   ), CHARSIZE=2, /DATA
    XYOUTS, 120, 1.06E4, 'h2o-nconv' , COLOR = COLOR_24('gray70'   ), CHARSIZE=2, /DATA
ENDIF
