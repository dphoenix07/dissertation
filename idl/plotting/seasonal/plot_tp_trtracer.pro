PRO PLOT_TP_TRTRACER, event, scheme, start_date, end_date, $
	PNG	       = png, $
	EPS   	   = eps
...
; Calling sequence:
;		PLOT_TP_TRTRACER, run, scheme, start_date, end_date
; Input:
...
outdir  = !WRF_DIRECTORY + event + '/' + scheme + '/plots/tropopause_tr_tracer/'
...
FOREACH date, date_arr DO BEGIN
	map_plot2 = (WRF_READ_VAR('Passive_TR_tracer'   , date, event, scheme, DOMAIN=domain, INDICES=region)).values
	...
	xyz_trop = REBIN(z_trop, dim[0],dim[1],dim[2],/SAMPLE)
 	...
 	map_plot = FLTARR(dim[0],dim[1]) * !Values.F_NaN

	;Find tr_tracer at tropopause 	 
	FOR ii = 0, dim[0]-1 DO BEGIN
		FOR jj = 0, dim[1]-1 DO BEGIN
			...
			index0 = index1[0]-5
			map_plot[ii,jj] = map_plot2[ii,jj,index0]
		ENDFOR
	ENDFOR
    ...
    ENDIF ELSE $	
    	 ...
    	 map_plot      = map_plot*100.0
    	 map_bar_title = 'TR tracer'														;Set color bar title
