PRO H2O_TIMESERIES_PLOTS, event, scheme, start_date, end_date, $
	DOMAIN   = domain, $
	REGION   = region, $
	TROP_REL = trop_rel, $
	REFL	 = refl, $
...
;								2018-03-15. Same as h2o_timeseries_350K for for 
;											380K - 420K layer.
;-
...
outdir  = !WRF_DIRECTORY + event + '/paper/plots/timeseries_new_380K/'
FILE_MKDIR, outdir
...
;	cld_arr = [ ]
 ...
 ;	ENDFOR
	...
	h2o_strat = WHERE(((theta.values GT 380.0) AND (theta.values LT 420.0)), sh2o_count, COMPLEMENT=h2o_trop)
...
FOREACH date, date_arr DO BEGIN
	...
	dim 	 = SIZE(z, /DIMENSIONS)																		;Get dimensions (to reform arrays later)
	PRINT, MAX(ztrop,/NAN), MIN(ztrop,/NAN)
...
;	CHARTHICK = 2.0, $
	...
	TITLE     = 'Convective Fraction vs Mean H2O Concentration (380K - 420K)'

AXIS, YAXIS = 0, $																								;Draw altitude axis
	SAVE     = 1, $
	YRANGE   = [0.0, 0.1], $
	YTITLE   = 'Convective Fraction (per thousand)', $
	YTICKS   = 1, $
	YSTYLE   = 1, $
	CHARSIZE = 3, $
	COLOR  	 = color0

PRINT, conv_fraction*1.0E4
OPLOT, time, conv_fraction*1.0E4, COLOR = color0, THICK = 3													;Plot temperature measurments
...
;			FOR k = 0, dim[2]-1 DO IF (theta.values[i,j,k] LT 350.0) THEN BEGIN
			FOR k = dim[2]-1, 0, -1 DO IF (theta.values[i,j,k] GT 380.0) THEN BEGIN
				ke = k
				ENDIF 
