PRO H2O_TIMESERIES_350K, event, scheme, start_date, end_date, $
	DOMAIN   = domain, $
	REGION   = region, $
	TROP_REL = trop_rel, $
	REFL	 = refl, $
...
;+
; Name:
;		H2O_TIMESERIES_PLOTS
; Purpose:
;		Computes fraction of domain where updraft_tracer = 1
; Calling sequence:
;		H2O_TIMESERIES_PLOTS, run, scheme, start_date, end_date
; Input:
...
;								2018-06-15. There are now two scripts, one to calculate
;											the necessary variables for plotting and write
;											to a file, and another script to read that file
;											and produce the plots. This script will do
;											both (and will be slower).
;-
...
;	cld_arr = [ ]
 ...
 ;	ENDFOR
	...
	strat_fraction = [TEMPORARY(strat_fraction), FLOAT((strat1 /total_size))] 
	...
	intensity1 = [TEMPORARY(intensity1), intensity]
	...
	trop_temp1 = [TEMPORARY(trop_temp1), MEAN(trop_temp,/NAN)]
	...
	PRINT, 'trop_temp (K) = ', MEAN(trop_temp,/NAN)	 
...
;	cld_arr = [ ]
 ...
 ;	ENDFOR
	...
	volume1 	   = TOTAL(conv_vol * z, /NAN)
	vol_size       = SIZE(conv_vol,/DIMENSIONS)
	strat1         = TOTAL(strat_vol * z, /NAN)
	total_size     = TOTAL(z, /NAN)
	conv_fraction  = [TEMPORARY(conv_fraction ), FLOAT((volume1/total_size))] 
	strat_fraction = [TEMPORARY(strat_fraction), FLOAT((strat1 /total_size))] 
...
AXIS, YAXIS = 0, $																								;Draw altitude axis
	SAVE     = 1, $
	YRANGE   = [0.0, 2.0], $
	YTITLE   = 'Convective Fraction/Intensity (%/km)', $
	YTICKS   = 1, $
	YSTYLE   = 1, $
	CHARSIZE = 3, $
	COLOR  	 = color0
...
OPLOT, time, intensity1*0.001, COLOR = color0, THICK = 3, LINESTYLE = 1												;Plot temperature measurments
...
OPLOT, time, strat_fraction*100.0, COLOR = color3, THICK = 3													;Plot temperature measurments
...
;			FOR k = 0, dim[2]-1 DO IF (theta.values[i,j,k] LT 350.0) THEN BEGIN
			...
			IF (z[i,j,ke] LT xyz_trop[i,j,ke]) THEN BEGIN
				h2o_slice  [i,j] = !Values.F_NaN
				cloud_slice[i,j] = !Values.F_NaN
			ENDIF
		ENDFOR
	ENDFOR
