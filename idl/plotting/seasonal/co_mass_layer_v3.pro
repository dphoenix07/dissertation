PRO CO_MASS_LAYER_V3, run, experiment, start_date, end_date, $
	DOMAIN        = domain, $
	REGION        = region, $
...
; Calling sequence:
;		CO_MASS_LAYER_V3, run, scheme, start_date, end_date
;		CO_MASS_LAYER_V3, '20120530_ncar','d03_30km','20120530T2200Z','20120531T0500Z'
; Input:
...
outdir  = !WRF_DIRECTORY + run + '/' + experiment + '/plots/co_mass_layer/'
FILE_MKDIR, outdir
...
FOREACH date, date_arr DO BEGIN
	...
	co    = (WRF_READ_VAR('CO'			   , date, run, experiment, DOMAIN = domain, INDICES = region)).values 
    ...
    carbonmo_ugm3  = (co * press*1.0E2 * molec_weight) / (r_star * temp)
    carbonmo_ug    = carbonmo_ugm3*dx*dx*250.0
    carbonmo_kg    = carbonmo_ug*1.0E-9
	carbonmo_kg_2d = FLTARR(dim[0],dim[1]) * !Values.F_NaN
	...
	FOR k = 0, dim[2]-1 DO BEGIN
		flux_u 		[k] = (MEAN((u[5,*,k]*carbonmo_kg[5,*,k] - u[dim[0]-5,*,k]*carbonmo_kg[dim[0]-5,*,k]),/NAN)) / ((dim[0] - 11) * dx)
		flux_v 		[k] = (MEAN((v[*,5,k]*carbonmo_kg[*,5,k] - v[*,dim[1]-5,k]*carbonmo_kg[*,dim[1]-5,k]),/NAN)) / ((dim[1] - 11) * dx)
		flux_u_tot  [k] = (TOTAL((u[5,*,k]*carbonmo_kg[5,*,k] - u[dim[0]-5,*,k]*carbonmo_kg[dim[0]-5,*,k]),/NAN)) / ((dim[0] - 11) * dx)
		flux_v_tot  [k] = (TOTAL((v[*,5,k]*carbonmo_kg[*,5,k] - v[*,dim[1]-5,k]*carbonmo_kg[*,dim[1]-5,k]),/NAN)) / ((dim[1] - 11) * dx)
	...
	carbonmo_conv  = FLTARR(dim[0],dim[1],dim[2])
	carbonmo_conv  = FLTARR(dim[0],dim[1],dim[2])
	carbonmo_nconv = FLTARR(dim[0],dim[1],dim[2])
	carbonmo_nconv = FLTARR(dim[0],dim[1],dim[2])
	
	convect = WHERE(updrt GT 0.25, count, COMPLEMENT = nconv)
	carbonmo_conv [convect] = carbonmo_kg [convect]
	carbonmo_conv [nconv  ] = !Values.F_NaN
	carbonmo_nconv[nconv  ] = carbonmo_kg[nconv]
	carbonmo_nconv[convect] = !Values.F_NaN	
	...
	carbonmo_kg_2d = carbonmo_kg[5:dim[0]-5,5:dim[1]-5,index0:index]
	co_conv_2d  = carbonmo_conv [5:dim[0]-5,5:dim[1]-5,index0:index]
	co_nconv_2d = carbonmo_nconv[5:dim[0]-5,5:dim[1]-5,index0:index]

   	PRINT, 'wrapping keyword not set'
   	inan    = WHERE(FINITE(co       	 [5:dim[0]-5,5:dim[1]-5,index0:index],/NAN),nan_count, COMPLEMENT = ivalue, NCOMPLEMENT = value_count)
   	inan_c  = WHERE(FINITE(carbonmo_conv [5:dim[0]-5,5:dim[1]-5,index0:index],/NAN),nan_count, COMPLEMENT = ivalue, NCOMPLEMENT = value_count_conv)
   	inan_nc = WHERE(FINITE(carbonmo_nconv[5:dim[0]-5,5:dim[1]-5,index0:index],/NAN),nan_count, COMPLEMENT = ivalue, NCOMPLEMENT = value_count_nconv)

	PRINT,'total convective carbonmo mass= ', (TOTAL(co_conv_2d ,/NAN))
	PRINT,'convective count= ', value_count_conv

	PRINT,'total non-convective carbonmo mass= ', (TOTAL(co_nconv_2d ,/NAN))
	PRINT,'non-convective count dracula= ', value_count_nconv

  	carbonmo_wrap   = (TOTAL(carbonmo_kg_2d,/NAN))/value_count
  	carbonmo_total  =  TOTAL(carbonmo_kg_2d,/NAN) 
	
	co_ave_conv  = (TOTAL(co_conv_2d ,/NAN))/value_count_conv
	co_ave_nconv = (TOTAL(co_nconv_2d,/NAN))/value_count_nconv
	co_tot_conv  = (TOTAL(co_conv_2d ,/NAN))
	co_tot_nconv = (TOTAL(co_nconv_2d,/NAN))	

    PRINT, date
    PRINT, 'Layer Limits: ', MEAN(z[*,*,index0],/NAN), MEAN(z[*,*,index],/NAN)
    carbonmo_array   = [carbonmo_array, carbonmo_wrap]
    carbonmo_tot_arr = [carbonmo_tot_arr, carbonmo_total]
    
    co_conv_arr   = [co_conv_arr  , co_ave_conv ]
    co_nconv_arr  = [co_nconv_arr , co_ave_nconv]
    co_conv_tarr  = [co_conv_tarr , co_tot_conv ]
    co_nconv_tarr = [co_nconv_tarr, co_tot_nconv]

    cloud_frac_tot = [cloud_frac_tot, cloud_fraction]
    
    PRINT, 'Convective mass= ', co_tot_conv 
    PRINT, 'Non-convective mass= ', co_tot_nconv 
    
    PRINT, 'Difference from total= ', (carbonmo_total - (co_tot_conv+co_tot_nconv))
...
outdir  = !WRF_DIRECTORY + run + '/' + experiment + '/plots/co_timeseries/'
...
dco_dt   = FLTARR(N_ELEMENTS(date_arr))
dcoc_dt  = FLTARR(N_ELEMENTS(date_arr))
dconc_dt = FLTARR(N_ELEMENTS(date_arr))
...
FOR tt = 0, N_ELEMENTS(date_arr) -3 DO BEGIN
	dco_dt[tt]   = (carbonmo_array[tt+1]   - carbonmo_array[tt]  ) / dt
	dcoc_dt[tt]  = (co_conv_arr[tt+1]   - co_conv_arr[tt]  ) / dt
	dconc_dt[tt] = (co_nconv_arr[tt+1] - co_nconv_arr[tt]) / dt
	;dco_dt[tt]   = (carbonmo_tot_arr[tt+1]   - carbonmo_tot_arr[tt]  ) / dt
	;dcoc_dt[tt]  = (co_conv_tarr[tt+1]   - co_conv_tarr[tt]  ) / dt
	;dconc_dt[tt] = (co_nconv_tarr[tt+1] - co_nconv_tarr[tt]) / dt
...
IF (run EQ '20120530_ncar') AND (experiment NE 'd02_30km') THEN BEGIN 
    ...
    OPLOT, SHIFT(dco_dt,1), COLOR = COLOR_24('red'), THICK=3
    OPLOT, time_layer_flux, COLOR = COLOR_24('blue'), THICK=3
    OPLOT, [0,250], [0,0], COLOR = COLOR_24('black'), THICK=3
    
    net = SHIFT(dco_dt,1) - time_layer_flux
	carbonmo_vertflux = carbonmo_tot_arr+time_layer_flux_tot*3600
	carbonmo_vertflux_conv =  co_conv_tarr +time_layer_flux_tot*3600
	carbonmo_vertflux_nconv = co_nconv_tarr+time_layer_flux_tot*3600
	...
	OPLOT, co_conv_arr,  COLOR = COLOR_24('gray30'), THICK=3, LINESTYLE=2
	OPLOT, co_nconv_arr, COLOR = COLOR_24('gray70'), THICK=3, LINESTYLE=2	    
    ;OPLOT, carbonmo_array, COLOR = COLOR_24('gray60'	 ), THICK=3
	...
	;OPLOT, co_conv_tarr,  COLOR = COLOR_24('gray30'), THICK=3, LINESTYLE=2
	;OPLOT, co_nconv_tarr, COLOR = COLOR_24('gray70'), THICK=3, LINESTYLE=2	    
    OPLOT, carbonmo_tot_arr, COLOR = COLOR_24('gray60'), THICK=3


    XYOUTS, 85, 7.5E7, '- dco/dt' , COLOR = COLOR_24('red'	  ), CHARSIZE=2, /DATA
    XYOUTS, 85, 7.1E7, '- net' 	, COLOR = COLOR_24('darkgreen'), CHARSIZE=2, /DATA
    XYOUTS, 85, 6.7E7, '- co flux', COLOR = COLOR_24('blue'     ), CHARSIZE=2, /DATA
    XYOUTS, 85, 6.3E7, '- Tot co' , COLOR = COLOR_24('gray60'   ), CHARSIZE=2, /DATA
    XYOUTS, 85, 5.9E7, 'co-conv'  , COLOR = COLOR_24('gray30'   ), CHARSIZE=2, /DATA
    XYOUTS, 85, 5.5E7, 'co-nconv' , COLOR = COLOR_24('gray70'   ), CHARSIZE=2, /DATA
ENDIF
...
;    	YRANGE 	 = [-600,600], $
 ...
 ; 		YRANGE   = [80, 130], $
	...
	OPLOT, carbonmo_vertflux_conv,  COLOR = COLOR_24('gray30'), THICK=3, LINESTYLE=2
	OPLOT, carbonmo_vertflux_nconv, COLOR = COLOR_24('gray70'), THICK=3, LINESTYLE=2	    
    OPLOT, carbonmo_vertflux, COLOR = COLOR_24('gray60'), THICK=3

    XYOUTS, 7.0, 7.5E7, '- dco/dt' , COLOR = COLOR_24('red'	  ), CHARSIZE=2, /DATA
    XYOUTS, 7.0, 7.1E7, '- net' 	, COLOR = COLOR_24('darkgreen'), CHARSIZE=2, /DATA
    XYOUTS, 7.0, 6.7E7, '- co flux', COLOR = COLOR_24('blue'     ), CHARSIZE=2, /DATA
    XYOUTS, 7.0, 6.3E7, '- Tot co' , COLOR = COLOR_24('gray60'   ), CHARSIZE=2, /DATA
    XYOUTS, 7.0, 5.9E7, 'co-conv'  , COLOR = COLOR_24('gray30'   ), CHARSIZE=2, /DATA
    XYOUTS, 7.0, 5.5E7, 'co-nconv' , COLOR = COLOR_24('gray70'   ), CHARSIZE=2, /DATA
ENDIF
...
IF (run EQ '20110518') THEN BEGIN 
    ...
    net = SHIFT(dco_dt,1) - time_layer_flux
	carbonmo_vertflux = carbonmo_tot_arr+time_layer_flux_tot*3600
	carbonmo_vertflux_conv =  co_conv_arr +time_layer_flux*3600
	carbonmo_vertflux_nconv = co_nconv_arr+time_layer_flux*3600
    OPLOT, net, COLOR = COLOR_24('darkgreen'), THICK=3
...
;    OPLOT, carbonmo_tot_arr, COLOR = COLOR_24('gray60'), THICK=3
    ...
    XYOUTS, 12, 5.98E8, '- dco/dt' , COLOR = COLOR_24('red'	  ), CHARSIZE=2, /DATA
    XYOUTS, 12, 5.97E8, '- net' 	, COLOR = COLOR_24('darkgreen'), CHARSIZE=2, /DATA
    XYOUTS, 12, 5.96E8, '- co flux', COLOR = COLOR_24('blue'     ), CHARSIZE=2, /DATA
    XYOUTS, 12, 5.95E8, '- Tot co' , COLOR = COLOR_24('gray60'   ), CHARSIZE=2, /DATA
    XYOUTS, 12, 5.94E8, 'co-conv'  , COLOR = COLOR_24('gray30'   ), CHARSIZE=2, /DATA
    XYOUTS, 12, 5.93E8, 'co-nconv' , COLOR = COLOR_24('gray70'   ), CHARSIZE=2, /DATA
ENDIF
...
;    OPLOT, carbonmo_tot_arr, COLOR = COLOR_24('gray60'), THICK=3
    ...
    XYOUTS, 12, 1.07E9, '- dco/dt' , COLOR = COLOR_24('red'	  ), CHARSIZE=2, /DATA
    XYOUTS, 12, 1.06E9, '- net' 	, COLOR = COLOR_24('darkgreen'), CHARSIZE=2, /DATA
    XYOUTS, 12, 1.05E9, '- co flux', COLOR = COLOR_24('blue'     ), CHARSIZE=2, /DATA
    XYOUTS, 12, 1.04E9, '- Tot co' , COLOR = COLOR_24('gray60'   ), CHARSIZE=2, /DATA
    XYOUTS, 12, 1.03E9, 'co-conv'  , COLOR = COLOR_24('gray30'   ), CHARSIZE=2, /DATA
    XYOUTS, 12, 1.02E9, 'co-nconv' , COLOR = COLOR_24('gray70'   ), CHARSIZE=2, /DATA
ENDIF
