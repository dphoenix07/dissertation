PRO WRITE_O3_THETA_TIMESERIES_FILE_V2, run, experiment, start_date, end_date, $
	DOMAIN        = domain, $
	REGION        = region, $
...
;+
; Name:
;		WRITE_O3_THETA_TIMESERIES_FILE_V2
; Purpose:
;		Calculates % mass (kg) of O3 in some layer
; Calling sequence:
;		WRITE_O3_THETA_TIMESERIES_FILE_V2, run, scheme, start_date, end_date
;		WRITE_O3_THETA_TIMESERIES_FILE_V2, '20120530_ncar','d03_30km','20120530T2200Z','20120531T0500Z'
; Input:
...
; Author and history:
;		Daniel B. Phoenix	    2019-08-27. Differs from v1 in that the % O3 mass is 
;											plotted
;	 
;-
...
;			index0 = MEAN(VALUE_LOCATE(REFORM(z[ii,jj,*],dim[2],1,1),REFORM(trop_3d[ii,jj,*],dim[2],1,1)))
			...
			;IF (ii EQ 	     5) THEN flux_u_left[*,jj,*]   = u[ii,jj,index0:(index1)]*o3_kg[ii,jj,index0:(index1)]
			;IF (ii EQ dim[0]-5) THEN flux_u_right[*,jj,*]  = u[ii,jj,index0:(index1)]*o3_kg[ii,jj,index0:(index1)]
			;IF (jj EQ 	     5) THEN flux_v_bottom[ii,*,*] = v[ii,jj,index0:(index1)]*o3_kg[ii,jj,index0:(index1)]
			;IF (jj EQ dim[1]-5) THEN flux_v_top[ii,*,*]    = v[ii,jj,index0:(index1)]*o3_kg[ii,jj,index0:(index1)]
			IF (ii EQ 	     5) THEN BEGIN
				nanl    = WHERE((u[ii,jj,index0:(index1)]*o3_kg[ii,jj,index0:(index1)]) NE 0.0, left_count0)
				nanairl = WHERE((u[ii,jj,index0:(index1)]*air_kg[ii,jj,index0:(index1)]) NE 0.0, airleft_count0)
				flux_u_left_ave    [*,jj] = TOTAL(u[ii,jj,index0:(index1)]*o3_kg [ii,jj,index0:(index1)],/NAN) / left_count0
				flux_u_left_tot    [*,jj] = TOTAL(u[ii,jj,index0:(index1)]*o3_kg [ii,jj,index0:(index1)],/NAN) 
				flux_air_left_ave  [*,jj] = TOTAL(u[ii,jj,index0:(index1)]*air_kg[ii,jj,index0:(index1)],/NAN) / airleft_count0
				flux_air_left_tot  [*,jj] = TOTAL(u[ii,jj,index0:(index1)]*air_kg[ii,jj,index0:(index1)],/NAN) 
			ENDIF
			IF (ii EQ dim[0]-5) THEN BEGIN
				nanr    = WHERE((u[ii,jj,index0:(index1)]*o3_kg [ii,jj,index0:(index1)]) NE 0.0, right_count0)
				nanairr = WHERE((u[ii,jj,index0:(index1)]*air_kg[ii,jj,index0:(index1)]) NE 0.0, airright_count0)
				flux_u_right_ave   [*,jj] = TOTAL(u[ii,jj,index0:(index1)]*o3_kg [ii,jj,index0:(index1)],/NAN) / right_count0
				flux_u_right_tot   [*,jj] = TOTAL(u[ii,jj,index0:(index1)]*o3_kg [ii,jj,index0:(index1)],/NAN) 
				flux_air_right_ave [*,jj] = TOTAL(u[ii,jj,index0:(index1)]*air_kg[ii,jj,index0:(index1)],/NAN) / airright_count0
				flux_air_right_tot [*,jj] = TOTAL(u[ii,jj,index0:(index1)]*air_kg[ii,jj,index0:(index1)],/NAN) 
			ENDIF
			IF (jj EQ 	     5) THEN BEGIN
				nanb 	= WHERE((v[ii,jj,index0:(index1)]*o3_kg [ii,jj,index0:(index1)]) NE 0.0, bottom_count0)
				nanairb = WHERE((v[ii,jj,index0:(index1)]*air_kg[ii,jj,index0:(index1)]) NE 0.0, airbottom_count0)
				flux_v_bottom_ave  [ii,*] = TOTAL(v[ii,jj,index0:(index1)]*o3_kg [ii,jj,index0:(index1)],/NAN) / bottom_count0
				flux_v_bottom_tot  [ii,*] = TOTAL(v[ii,jj,index0:(index1)]*o3_kg [ii,jj,index0:(index1)],/NAN) 
				flux_air_bottom_ave[ii,*] = TOTAL(v[ii,jj,index0:(index1)]*air_kg[ii,jj,index0:(index1)],/NAN) / airbottom_count0
				flux_air_bottom_tot[ii,*] = TOTAL(v[ii,jj,index0:(index1)]*air_kg[ii,jj,index0:(index1)],/NAN) 
			ENDIF
			IF (jj EQ dim[1]-5) THEN BEGIN
				nant 	= WHERE((v[ii,jj,index0:(index1)]*o3_kg [ii,jj,index0:(index1)]) NE 0.0, top_count0)
				nanairt = WHERE((v[ii,jj,index0:(index1)]*air_kg[ii,jj,index0:(index1)]) NE 0.0, airtop_count0)
				flux_v_top_ave   [ii,*] = TOTAL(v[ii,jj,index0:(index1)]*o3_kg [ii,jj,index0:(index1)],/NAN) / top_count0
				flux_v_top_tot   [ii,*] = TOTAL(v[ii,jj,index0:(index1)]*o3_kg [ii,jj,index0:(index1)],/NAN) 
				flux_air_top_ave [ii,*] = TOTAL(v[ii,jj,index0:(index1)]*air_kg[ii,jj,index0:(index1)],/NAN) / airtop_count0
				flux_air_top_tot [ii,*] = TOTAL(v[ii,jj,index0:(index1)]*air_kg[ii,jj,index0:(index1)],/NAN) 
			ENDIF

			o3_kg_2d    [ii,jj] = TOTAL(o3_kg [ii,jj,index0:(index1)])
			air_kg_2d   [ii,jj] = TOTAL(air_kg[ii,jj,index0:(index1)])
			...
			FOR kk = 0, nz-1 DO BEGIN
				IF (updrt[ii,jj,kk+index0] GE 0.25) THEN o3_conv  [kk] = o3_kg  [ii,jj,kk+index0]
				IF (updrt[ii,jj,kk+index0] LT 0.25) THEN o3_nconv [kk] = o3_kg  [ii,jj,kk+index0]
				IF (updrt[ii,jj,kk+index0] GE 0.25) THEN air_conv [kk] = air_kg [ii,jj,kk+index0]
				IF (updrt[ii,jj,kk+index0] LT 0.25) THEN air_nconv[kk] = air_kg [ii,jj,kk+index0]
			ENDFOR
			...
			inan     = WHERE(FINITE(o3_nan      [ii,jj,index0:(index1)],/NAN),nan_count, COMPLEMENT = ivalue, NCOMPLEMENT = value_count1)
			iairnan  = WHERE(FINITE(air_nan     [ii,jj,index0:(index1)],/NAN),nan_count, COMPLEMENT = ivalue, NCOMPLEMENT = airvalue_count1)
...
outdir  = !WRF_DIRECTORY + run + '/' + experiment + '/plots/o3_timeseries_theta_v2/'
...
FOR tt = 0, N_ELEMENTS(date_arr) -3 DO BEGIN
	...
	dperc_dt[tt] = ((o3_array[tt+1]/air_array[tt+1]) - (o3_array[tt]/air_array[tt])) / dt
...
IF (run EQ '20110518') THEN BEGIN 
    PLOT, SHIFT((do3_dt/dair_dt),1), $
    	YRANGE 	 = [-2E-7,2E-7], $
    	YTITLE   = 'air (kg/s)', $
    	COLOR    = COLOR_24('black'), $
    	TITLE    = 'may air Mass (kg)', $
 		XTICKINTERVAL = 12, $
    	CHARSIZE = 2, $
    	/NODATA
   ...
   OPLOT, SHIFT((do3_dt/dair_dt),1), COLOR = COLOR_24('red'), THICK=2
   OPLOT, (time_flux/time_air_flux), COLOR = COLOR_24('blue'), THICK=3
   OPLOT, [0,250], [0,0], COLOR = COLOR_24('black'), THICK=2
    
    net = SHIFT((do3_dt/dair_dt),1) - (time_flux/time_air_flux)
	o3_vertflux = (o3_tot_arr/air_tot_arr);+(time_flux_tot/time_air_flux_tot)*3600
	o3_vertflux_conv =  (o3_conv_tarr/air_conv_tarr); +(time_flux_tot/time_air_flux_tot)*3600
	o3_vertflux_nconv = (o3_nconv_tarr/air_nconv_tarr);+(time_flux_tot/time_air_flux_tot)*3600
    ...
    AXIS, YAXIS = 1, $																								;Draw altitude axis
    	SAVE     = 1, $
    ;	YRANGE   = [4.0E-9,7.0E-9], $
    	YRANGE   = [4.0E-9,5.5E-9], $
    	YTITLE   = 'air (kg)', $
    	COLOR    = COLOR_24('black'), $
    	YTICKS   = 4, $
    	YSTYLE   = 1, $
...
;    	YRANGE   = [4.5E-9,7.0E-9], $
  ...
  XYOUTS, 12, 1.95E4, '- do3/dt' , COLOR = COLOR_24('red'	   ), CHARSIZE=2, /DATA
  XYOUTS, 12, 1.90E4, '- net' 	 , COLOR = COLOR_24('darkgreen'), CHARSIZE=2, /DATA
  XYOUTS, 12, 1.85E4, '- o3 flux', COLOR = COLOR_24('blue'     ), CHARSIZE=2, /DATA
  XYOUTS, 12, 1.80E4, '- Tot o3' , COLOR = COLOR_24('gray60'   ), CHARSIZE=2, /DATA
  XYOUTS, 12, 1.75E4, 'o3-conv'  , COLOR = COLOR_24('gray30'   ), CHARSIZE=2, /DATA
  XYOUTS, 12, 1.70E4, 'o3-nconv' , COLOR = COLOR_24('gray70'   ), CHARSIZE=2, /DATA
ENDIF
...
fname = outdir+run+'_perco3_ztrop.txt'
OPENW, lun, fname, /GET_LUN     

PRINTF, lun, FORMAT = '("  Date", 6X, "d%O3/dt", 6X, "Horiz. Flux", 6X, "Vert. Flux", 6X, "%O3-Conv.", 6X, "%O3-NConv.", 6X, "%O3-Total")'								
PRINTF, lun, '========================================================================================================='

FOR i = 0, N_ELEMENTS(date_arr)-1 DO BEGIN
	PRINTF, lun, FORMAT = '(I3, 4X, E12.4, 4X, E12.4, 4X, E12.4, 4X, E12.4, 4X, E12.4, 4X, E12.4)', i,(do3_dt[i]/dair_dt[i]),(time_flux[i]/time_air_flux[i]), net[i],o3_vertflux_conv[i], o3_vertflux_nconv[i],o3_vertflux[i]	
ENDFOR
FREE_LUN, lun
