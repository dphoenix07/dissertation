PRO WRITE_H2O_TIMESERIES_FILE, run, experiment, start_date, end_date, $
	DOMAIN        = domain, $
	REGION        = region, $
...
FOREACH date, date_arr DO BEGIN
	...
	FOR ii = 5, dim[0]-5 DO BEGIN
		FOR jj = 5, dim[1]-5 DO BEGIN
			...
			IF (ii EQ 	     5) THEN flux_u_left[*,jj,*]   = u[ii,jj,index0:(index0+10)]*h2o_kg[ii,jj,index0:(index0+10)]
			IF (ii EQ dim[0]-5) THEN flux_u_right[*,jj,*]  = u[ii,jj,index0:(index0+10)]*h2o_kg[ii,jj,index0:(index0+10)]
			IF (jj EQ 	     5) THEN flux_v_bottom[ii,*,*] = v[ii,jj,index0:(index0+10)]*h2o_kg[ii,jj,index0:(index0+10)]
			IF (jj EQ dim[1]-5) THEN flux_v_top[ii,*,*]    = v[ii,jj,index0:(index0+10)]*h2o_kg[ii,jj,index0:(index0+10)]
...
IF (run EQ '20110518') THEN BEGIN 
  ...
  XYOUTS, 12, 3.95E4, '- dh2o/dt' , COLOR = COLOR_24('red'	  ), CHARSIZE=2, /DATA
  XYOUTS, 12, 3.90E4, '- net' 	, COLOR = COLOR_24('darkgreen'), CHARSIZE=2, /DATA
  XYOUTS, 12, 3.85E4, '- h2o flux', COLOR = COLOR_24('blue'     ), CHARSIZE=2, /DATA
  XYOUTS, 12, 3.80E4, '- Tot h2o' , COLOR = COLOR_24('gray60'   ), CHARSIZE=2, /DATA
  XYOUTS, 12, 3.75E4, 'h2o-conv'  , COLOR = COLOR_24('gray30'   ), CHARSIZE=2, /DATA
  XYOUTS, 12, 3.70E4, 'h2o-nconv' , COLOR = COLOR_24('gray70'   ), CHARSIZE=2, /DATA
ENDIF
...
fname = outdir+run+'_h2o_ztrop.txt'
