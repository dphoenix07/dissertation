PRO WRITE_CO_THETA_TIMESERIES_FILE, run, experiment, start_date, end_date, $
	DOMAIN        = domain, $
	REGION        = region, $
...
;+
; Name:
;		WRITE_CO_THETA_TIMESERIES_FILE
; Purpose:
;		Calculates mass (kg) of O3 in some layer
; Calling sequence:
;		WRITE_CO_THETA_TIMESERIES_FILE, run, scheme, start_date, end_date
;		WRITE_CO_THETA_TIMESERIES_FILE, '20120530_ncar','d03_30km','20120530T2200Z','20120531T0500Z'
; Input:
...
outdir  = !WRF_DIRECTORY + run + '/' + experiment + '/reduced/co_mass_layer_theta/'
FILE_MKDIR, outdir
...
FOREACH date, date_arr DO BEGIN
    ...
    co_ugm3        = (co * press*1.0E2 * molec_weight) / (r_star * temp)
    co_ug          = co_ugm3*dx*dx*250.0
    co_kg          = co_ug*1.0E-9
	co_kg_2d       = FLTARR(dim[0],dim[1]) * !Values.F_NaN
	co_kg_ave      = FLTARR(dim[0],dim[1]) * !Values.F_NaN
	...
	co_conv_2d  = FLTARR(dim[0],dim[1])
	co_nconv_2d = FLTARR(dim[0],dim[1])
...
;			index0 = MEAN(VALUE_LOCATE(REFORM(z[ii,jj,*],dim[2],1,1),REFORM(trop_3d[ii,jj,*],dim[2],1,1)))
			...
			;IF (ii EQ 	     5) THEN flux_u_left[*,jj,*]   = u[ii,jj,index0:(index1)]*co_kg[ii,jj,index0:(index1)]
			;IF (ii EQ dim[0]-5) THEN flux_u_right[*,jj,*]  = u[ii,jj,index0:(index1)]*co_kg[ii,jj,index0:(index1)]
			;IF (jj EQ 	     5) THEN flux_v_bottom[ii,*,*] = v[ii,jj,index0:(index1)]*co_kg[ii,jj,index0:(index1)]
			;IF (jj EQ dim[1]-5) THEN flux_v_top[ii,*,*]    = v[ii,jj,index0:(index1)]*co_kg[ii,jj,index0:(index1)]
			IF (ii EQ 	     5) THEN BEGIN
				nanl = WHERE((u[ii,jj,index0:(index1)]*co_kg[ii,jj,index0:(index1)]) NE 0.0, left_count0)
				flux_u_left_ave  [*,jj] = TOTAL(u[ii,jj,index0:(index1)]*co_kg[ii,jj,index0:(index1)],/NAN) / left_count0
				flux_u_left_tot  [*,jj] = TOTAL(u[ii,jj,index0:(index1)]*co_kg[ii,jj,index0:(index1)],/NAN) 
			ENDIF
			IF (ii EQ dim[0]-5) THEN BEGIN
				nanr = WHERE((u[ii,jj,index0:(index1)]*co_kg[ii,jj,index0:(index1)]) NE 0.0, right_count0)
				flux_u_right_ave [*,jj] = TOTAL(u[ii,jj,index0:(index1)]*co_kg[ii,jj,index0:(index1)],/NAN) / right_count0
				flux_u_right_tot [*,jj] = TOTAL(u[ii,jj,index0:(index1)]*co_kg[ii,jj,index0:(index1)],/NAN) 
			ENDIF
			IF (jj EQ 	     5) THEN BEGIN
				nanb = WHERE((v[ii,jj,index0:(index1)]*co_kg[ii,jj,index0:(index1)]) NE 0.0, bottom_count0)
				flux_v_bottom_ave[ii,*] = TOTAL(v[ii,jj,index0:(index1)]*co_kg[ii,jj,index0:(index1)],/NAN) / bottom_count0
				flux_v_bottom_tot[ii,*] = TOTAL(v[ii,jj,index0:(index1)]*co_kg[ii,jj,index0:(index1)],/NAN) 
			ENDIF
			IF (jj EQ dim[1]-5) THEN BEGIN
				nant = WHERE((v[ii,jj,index0:(index1)]*co_kg[ii,jj,index0:(index1)]) NE 0.0, top_count0)
				flux_v_top_ave   [ii,*] = TOTAL(v[ii,jj,index0:(index1)]*co_kg[ii,jj,index0:(index1)],/NAN) / top_count0
				flux_v_top_tot   [ii,*] = TOTAL(v[ii,jj,index0:(index1)]*co_kg[ii,jj,index0:(index1)],/NAN) 
			ENDIF

			co_kg_2d    [ii,jj] = TOTAL(co_kg[ii,jj,index0:(index1)])
			
			co_conv  = FLTARR(nz)
			co_nconv = FLTARR(nz)
	
			FOR kk = 0, nz-1 DO BEGIN
				IF (updrt[ii,jj,kk+index0] GE 0.25) THEN co_conv  [kk] = co_kg [ii,jj,kk+index0]
				IF (updrt[ii,jj,kk+index0] LT 0.25) THEN co_nconv [kk] = co_kg [ii,jj,kk+index0]
			ENDFOR
			
			co_conv_2d  [ii,jj] = TOTAL(co_conv ,/NAN)
			co_nconv_2d [ii,jj] = TOTAL(co_nconv,/NAN)
			
			inan     = WHERE(FINITE(co_nan      [ii,jj,index0:(index1)],/NAN),nan_count, COMPLEMENT = ivalue, NCOMPLEMENT = value_count1)
			;inan_c   = WHERE(FINITE(co_conv [ii,jj,index0[ii,jj]:(index0[ii,jj]+10)],/NAN),nan_count, COMPLEMENT = ivalue, NCOMPLEMENT = value_count_conv1)
			;inan_nc  = WHERE(FINITE(co_nconv[ii,jj,index0[ii,jj]:(index0[ii,jj]+10)],/NAN),nan_count, COMPLEMENT = ivalue, NCOMPLEMENT = value_count_nconv1)
			value_count [ii,jj] = value_count1			

			co_kg_ave [ii,jj] = co_kg_2d [ii,jj] / value_count1
		ENDFOR
	ENDFOR			
	...
	time_flux_tot = [time_flux_tot, flux_tot_tot]
  	...
  	nan_ave = WHERE(FINITE(co_kg_ave,/NAN),nan_count, COMPLEMENT=ivalue, NCOMPLEMENT = value_count)
  	
	co_array   = [co_array  , TOTAL(co_kg_ave,/NAN)/value_count]
	co_tot_arr = [co_tot_arr, TOTAL(co_kg_2d,/NAN)]
	
	;co_conv_arr   = [co_conv_arr  , co_ave_conv ]
	;co_nconv_arr  = [co_nconv_arr , co_ave_nconv]
	co_conv_tarr  = [co_conv_tarr , TOTAL(co_conv_2d ,/NAN)]
	co_nconv_tarr = [co_nconv_tarr, TOTAL(co_nconv_2d,/NAN)]
...
outdir  = !WRF_DIRECTORY + run + '/' + experiment + '/plots/co_timeseries_theta/'
...
IF (run EQ '20110518') THEN BEGIN 
   ...
   OPLOT, SHIFT(dco_dt,1), COLOR = COLOR_24('red'), THICK=2
   OPLOT, time_flux, COLOR = COLOR_24('blue'), THICK=3
   OPLOT, [0,250], [0,0], COLOR = COLOR_24('black'), THICK=2
    
    net = SHIFT(dco_dt,1) - time_flux
	co_vertflux = co_tot_arr+time_flux_tot*3600
	co_vertflux_conv =  co_conv_tarr +time_flux_tot*3600
	co_vertflux_nconv = co_nconv_tarr+time_flux_tot*3600
    OPLOT, net, COLOR = COLOR_24('darkgreen'), THICK=3
    
    ;add convective fraction line from H2O timeseries in AI
     ;AXIS, YAXIS = 0, $																								;Draw altitude axis
    ;	SAVE     = 1, $
    ;	YRANGE 	 = [10.0, 30.0], $
	...
	OPLOT, co_vertflux_conv,  COLOR = COLOR_24('gray30'), THICK=3, LINESTYLE=2
	OPLOT, co_vertflux_nconv, COLOR = COLOR_24('gray70'), THICK=3, LINESTYLE=2	    
    
    ;;OPLOT, co_array, COLOR = COLOR_24('gray60'	 ), THICK=3
    ...
    ;OPLOT, co_tot_arr, COLOR = COLOR_24('gray60'), THICK=3
    OPLOT, co_vertflux, COLOR = COLOR_24('gray60'), THICK=5
	...
	; XYOUTS, 12, 2.75E4,'- Tot co'   , COLOR = COLOR_24('gray60'   ), CHARSIZE=2, /DATA
	; XYOUTS, 12, 2.70E4,'conv. fraction' , COLOR = COLOR_24('black'   ), CHARSIZE=2, /DATA

  XYOUTS, 12, 1.95E4, '- dco/dt' , COLOR = COLOR_24('red'	  ), CHARSIZE=2, /DATA
  XYOUTS, 12, 1.90E4, '- net' 	, COLOR = COLOR_24('darkgreen'), CHARSIZE=2, /DATA
  XYOUTS, 12, 1.85E4, '- co flux', COLOR = COLOR_24('blue'     ), CHARSIZE=2, /DATA
  XYOUTS, 12, 1.80E4, '- Tot co' , COLOR = COLOR_24('gray60'   ), CHARSIZE=2, /DATA
  XYOUTS, 12, 1.75E4, 'co-conv'  , COLOR = COLOR_24('gray30'   ), CHARSIZE=2, /DATA
  XYOUTS, 12, 1.70E4, 'co-nconv' , COLOR = COLOR_24('gray70'   ), CHARSIZE=2, /DATA
ENDIF
...
IF (run EQ '20130805') THEN BEGIN 
  ...
  XYOUTS, 192, 8.00E3, '- dco/dt' , COLOR = COLOR_24('red'	  ), CHARSIZE=2, /DATA
  XYOUTS, 192, 7.50E3, '- net' 	, COLOR = COLOR_24('darkgreen'), CHARSIZE=2, /DATA
  XYOUTS, 192, 7.00E3, '- co flux', COLOR = COLOR_24('blue'     ), CHARSIZE=2, /DATA
  XYOUTS, 192, 6.50E3, '- Tot co' , COLOR = COLOR_24('gray60'   ), CHARSIZE=2, /DATA
  XYOUTS, 192, 6.00E3, 'co-conv'  , COLOR = COLOR_24('gray30'   ), CHARSIZE=2, /DATA
  XYOUTS, 192, 5.50E3, 'co-nconv' , COLOR = COLOR_24('gray70'   ), CHARSIZE=2, /DATA
ENDIF
