PRO MAP_OZONE_ISENTROPE, event, scheme, start_date, end_date, isen, $
	DOMAIN   = domain, $
	REGION   = region, $
	TRACER   = tracer, $
	PNG	     = png, $
...
; Calling sequence:
;		MAP_OZONE_ISENTROPE, run, scheme, start_date, end_date, isen
; Input:
...
FOREACH date, date_arr DO BEGIN
    ...
    xyz_trop[filt] = !Values.F_NaN
 	...
 	map_sfc = FLTARR(dim[0],dim[1],dim[2]) + isen
 	 
	FOR ii = 0, dim[0]-1 DO BEGIN
		FOR jj = 0, dim[1]-1 DO BEGIN
			index = VALUE_LOCATE(REFORM(theta[ii,jj,*],dim[2],1,1),REFORM(map_sfc[ii,jj,*],dim[2],1,1))
			map_o3 [ii,jj] = o3_tr[ii,jj,index[0]]
			
;			FOR kk = 0, dim[2]-1 DO BEGIN
;				IF (ABS(map_sfc[ii,jj,kk] - theta[ii,jj,kk]) LT 2.0) THEN BEGIN
;					PRINT, ii, jj, kk, ' Tropopause identified = ', theta[ii,jj,kk]
;					map_o3 [ii,jj] = o3_tr[ii,jj,kk]
;					kk = dim[2]-1
;				ENDIF
;			ENDFOR
		ENDFOR
...
ENDIF ELSE $	
	 ...
	 map_bar_title = 'Tracer = ' + STRING(tracer) + ' Surface =' + STRING(isen) 														;Set color bar title
	 map_bar_min   = 0.0																						;Set echo top minimum
	 map_bar_max   = 200.0																					;Set echo top maximum
