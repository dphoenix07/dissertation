...
; 			 map_plot	   = (var.values[*,*,0]*1.0E3)
    		 ...
    		 map_bar_title = STRING(chemical)																;Set color bar title
    		 map_bar_min   = -5.0																			;Set tracer minimum
    		 map_bar_max   = 5.0																			;Set tracer maximum
...
CASE section_type OF
	0 : BEGIN
		 ...
		 ELSE BEGIN
			...
			var.values [good] = var.values [good]
...
outdir  = !WRF_DIRECTORY + experiment + '/paper/plots/xsect_final/srhwind/' 
...
;
;cont_plot  = INTERPOLATE(var,   iisect, jjsect, kksect, MISSING = !Values.F_NaN)					;Interpolate potential temperature to contour cross-section
;cont_plot_color  = COLOR_24('black')																;Set contour cross-section specs
;cont_plot_levels = 25.0*FINDGEN(40)
;cont_plot_thick  = 1
;
;cont_plot2 = CONGRID(cont_plot, xsize, ysize)                                                                                     ;Resample image
;cont_plot  = INTERPOLATE(cont_plot, MAKEN(0, nsect-1, xsize), MAKEN(0, nzsect-1, ysize), /GRID) ;Interpolate to high resolution
;inan          = WHERE((FINITE(cont_plot2) AND ~FINITE(cont_plot)), nnan)          ;Find points lost to interpolation
;IF (nnan GT 0) THEN cont_plot[inan] = cont_plot2[inan]                                                            ;Fix points
;
;CONTOUR, cont_plot, FINDGEN(xsize),  MAKEN(zrange[0], zrange[1], ysize), $															;Overplot contour cross-section
;	;OVERPLOT = 1, $
;	LEVELS   = [100.0,125.0,150.0], $;[100.0, 150.0, 200.0, 250.0], $
;	C_COLOR  = cont_plot_color, $
;	C_THICK  = cont_plot_thick*thick_scale*3.0, $
;	NODATA   = 1, $
;	DEVICE   = 1, $
;    POSITION = xsn_pos
    ;CELL_FILL = 1, $
...
table = [HCL_COLOR_TABLE(25, HUE_RANGE = [220.0,220.0], SAT_RANGE = [0.2,1.0], /REV), $
               HCL_COLOR_TABLE(25, HUE_RANGE = [  0.0,  0.0], SAT_RANGE = [0.2,1.0])]
