PRO PLOT_WRF_SECTION_DIFFERENCE, experiment, x0, y0, x1, y1, date, $
	DOMAIN       = domain, $
	MAP_TYPE     = map_type, $
...
; Purpose:
;		This is a procedure to plot a vertical cross-section difference between icloud
;		simulations for 30 May 2012 hi-res simulation
; Calling sequence:
...
IF (N_ELEMENTS(date        ) EQ 0) THEN date         = MAKE_DATE(2012, 5, 31, 1, 00)			;Set default input variables
IF (N_ELEMENTS(experiment  ) EQ 0) THEN experiment   = '20120530_ncar'
...
IF (N_ELEMENTS(map_type    ) EQ 0) THEN map_type     = 2
IF (N_ELEMENTS(section_type) EQ 0) THEN section_type = 4
...
state_array = ['d03_30km', 'd03_30km_icloud']
st=0
FOREACH state, state_array DO BEGIN
	...
	PRINT, x.values[x0,y0], y.values[x0,y0]
	PRINT
	PRINT, 'Section End (Lon, Lat): '
	PRINT, x.values[x1,y1], y.values[x1,y1]
	...
	CASE map_type OF
		0 : BEGIN
			 ...
			 ;map_table     = [COLOR_24(256, 256, 256),VISUALIZE_88D_COLOR(0)]							;Set color table
			 map_table     = [COLOR_24(256, 256, 256),GRIDRAD_VIEWER_COLOR(1)]							;Set color table
	...
	;    		 map_plot      = (var.values[*,*,50]*1.0E11)					;for OH							;Compute max tracer concentration
				 map_plot	   = (var.values[*,*,43])
	; 			 map_plot      = MAX(var.values, DIM = 3, /NAN)
				 map_bar_title = STRING(chemical)																;Set color bar title
				 map_bar_min   = 0.0																			;Set tracer minimum
				 map_bar_max   = 100.0																			;Set tracer maximum
	...
	CASE section_type OF
		0 : BEGIN	;Reflectivity
			 ...
			 ;table  		  = VISUALIZE_88D_COLOR(0)														;Set color-fill cross-section specs
			 table   		  = [COLOR_24(256, 256, 256),GRIDRAD_VIEWER_COLOR(1)]												;Set reflectivity color table
	;		 table   		  = [COLOR_24(230, 230, 230),VISUALIZE_88D_COLOR(0)	]												;Set reflectivity color table
			 ;table   		  = GRIDRAD_VIEWER_COLOR(1)												;Set reflectivity color table
			 fill_sect_levels =	[-100.0, 5.0*FINDGEN(N_ELEMENTS(table))]								;Set contour levels
			 sect_bar_min     = 0.0	
			 sect_bar_max     = 75.0
			 sect_bar_ticks   = 5	
			 ...
			 END
		1 : BEGIN	;Cloud Particle Concentration
			 ...
			 table  		  = [COLOR_24('white'), HCL_COLOR_TABLE(7, $								;Set color-fill cross-section specs
											HUE_RANGE = [180.0, 260.0], SAT_RANGE = [0.2,0.6])]
			...
			ENDIF
			 ...
			 CASE chemical OF
				'O3' : BEGIN 
						 ...
						 table  		  = HCL_COLOR_TABLE(20, HUE_RANGE = [100.0, 300.0])			;Set color-fill cross-section specs
						 sect_bar_min     = 0.0	
						 sect_bar_max     = 200.0
				...
				'H2O' : BEGIN 
						 ...
						 END
				;'H2O' : BEGIN 
				;		 press = (WRF_READ_VAR('P', date, experiment, state, DOMAIN = domain, INDICES = region)).values							
				;		 temp  = (WRF_READ_VAR('T', date, experiment, state, DOMAIN = domain, INDICES = region)).values						
			   ;         molec_weight = 18.0
			   ;         r_star = 8.314
			   ;         dx = 3000.0
				;		 fill_sect        = ((1.0E6)*fill_sect* press*1.0E2 * molec_weight*dx*dx*250.0) / (r_star * temp)													;Convert to ppbv
				;		 fill_sect_table  = HCL_COLOR_TABLE(20, HUE_RANGE = [100.0, 300.0])			;Set color-fill cross-section specs
				;		 sect_bar_min     = 3.4E10
				;		 sect_bar_max     = 2.7E13
				;		 fill_sect_levels = ((sect_bar_max-sect_bar_min)/20.0)*FINDGEN(20)
				...
				'ST_tracer' : BEGIN
						 ...
						 sect_bar_min     = 0.0	
						 sect_bar_max     = 400.0
						 fill_sect_levels = sect_bar_min+((sect_bar_max-sect_bar_min)/20.0)*FINDGEN(20)
						 sect_bar_ticks   = 4
				...
				'Passive_TR_tracer' : BEGIN
						 ...
						 fill_sect_levels = ((sect_bar_max-sect_bar_min)/20.0)*FINDGEN(20)
						 sect_bar_ticks   = 4
						 sect_bar_title   = 'Passive_TR_tracer'
						 END

				'Passive_Overworld_ST_tracer' : BEGIN
						 fill_sect        = (100.0)*fill_sect													;Convert to ppbv
						 fill_sect_table  = HCL_COLOR_TABLE(20, HUE_RANGE = [100.0, 300.0])			;Set color-fill cross-section specs
						 table  		  = HCL_COLOR_TABLE(20, HUE_RANGE = [100.0, 300.0])			;Set color-fill cross-section specs
						 sect_bar_min     = 0.0	
						 ...
						 fill_sect_levels = ((sect_bar_max-sect_bar_min)/20.0)*FINDGEN(20)
						 sect_bar_ticks   = 4
						 sect_bar_title   = 'Passive_Overworld_ST_tracer'
						 END
	...
	;					 fill_sect_table  = BLUE_RED_24(N_ELEMENTS(fill_sect_levels))
						 ...
						 table  		  = HCL_COLOR_TABLE(N_ELEMENTS(fill_sect_levels), HUE_RANGE = [100.0, 300.0])			;Set color-fill cross-section specs
						 sect_bar_ticks   = 4
	...
	;					 var =  (var - o3)
		...
		3 : BEGIN	;Storm-relative horizontal winds (user enters storm motion on next line)
			 storm_motion = 21.0
			 ...
			 table = [HCL_COLOR_TABLE(10, HUE_RANGE = [220.0,220.0], SAT_RANGE = [0.2,1.0], /REV), $
				   HCL_COLOR_TABLE(10, HUE_RANGE = [  0.0,  0.0], SAT_RANGE = [0.2,1.0])]
			 sect_bar_ticks   = 4
			 sect_bar_title   = 'Storm Relative Horizontal Wind Speed (m/s)'		 
		
			END	
			 
		4 : BEGIN	;Static stability
			 storm_motion = 21.0
	...
	;		 sect_bar_max     =  0.02
			...
			; sect_bar_min     = -0.12
			; sect_bar_max     =  0.12
			 fill_sect_table  = BLUE_RED_24(20)
	;		 table = [HCL_COLOR_TABLE(10, HUE_RANGE = [220.0,220.0], SAT_RANGE = [0.2,1.0], /REV), $
	;          		  HCL_COLOR_TABLE(20, HUE_RANGE = [  0.0,  0.0], SAT_RANGE = [0.2,1.0])]
			 table = [HCL_COLOR_TABLE(10 , HUE_RANGE = [220.0,220.0], SAT_RANGE = [0.2,1.0], /REV), $
	          		  HCL_COLOR_TABLE(21, HUE_RANGE = [  0.0,  0.0], SAT_RANGE = [0.2,1.0])]
	;		 table = [HCL_COLOR_TABLE(1 , HUE_RANGE = [256.0,256.0], SAT_RANGE = [0.0,0.0], /REV), $
	;			  HCL_COLOR_TABLE(29, HUE_RANGE = [  0.0,  0.0], SAT_RANGE = [0.2,1.0])]
			 ;table = BLUE_RED_24(20)

			 fill_sect_levels = MAKEN(-0.050, 0.120, 31)
			 ;fill_sect_levels = MAKEN(-0.120, 0.120, 30)
	;		 fill_sect_levels = MAKEN(-0.0007, 0.020, 30)
			 sect_bar_title   = 'Potential Temperature Gradient Difference (K/km)'		 
			
			END		 

		5 : BEGIN	;Richardson number (exercise caution when using this; may contain bugs)
			 z 	   =(WRF_READ_VAR('Z', date, experiment, state, DOMAIN = domain)).values						;Read z variable from WRF output
			 theta = WRF_READ_VAR('T', date, experiment, state, DOMAIN = domain)								;Read temperature variable from WRF output
			 theta.values = ((1000.0/(WRF_READ_VAR('P', date, experiment, state, $								;Compute potential temperature
								DOMAIN = domain)).values)^(!Rair/!Cp))*(theta.values)
			 u  = (WRF_READ_VAR('u'  , date, experiment, state, DOMAIN = domain)).values						;Read temperature variable from WRF output
			 v  = (WRF_READ_VAR('v'  , date, experiment, state, DOMAIN = domain)).values	
			 qv = (WRF_READ_VAR('H2O', date, experiment, state, DOMAIN = domain)).values	
			 t  = WRF_READ_VAR('T', date, experiment, state, DOMAIN = domain)								;Read temperature variable from WRF output
			 thetav = theta.values*(1.0 + 0.61*qv)
			 tempv  = t.values*(1.0 + 0.61*qv)
			 ...
			 z_sect      = INTERPOLATE(z     , iisect, jjsect, kksect, MISSING = !Values.F_NaN)
			 thetav_sect = INTERPOLATE(thetav, iisect, jjsect, kksect, MISSING = !Values.F_NaN)
			 tempv_sect  = INTERPOLATE(tempv , iisect, jjsect, kksect, MISSING = !Values.F_NaN)
	 
			 dim = SIZE(z_sect,/DIMENSIONS)        
			 ri  = FLTARR(dim[0],dim[1])*!Values.F_NaN
			 FOR i = 0, dim[0]-1 DO BEGIN
				FOR j = 0, dim[1]-1 DO BEGIN
					IF (j LT dim[1]-2) THEN $
						ri[i,j] = ((9.81/tempv_sect[i,j])*(thetav_sect[i,j+1]-thetav_sect[i,j-1])*(z_sect[i,j+1]-z_sect[i,j-1])) / ((u_sect[i,j+1]-u_sect[i,j-1])^2 + (v_sect[i,j+1]-v_sect[i,j-1])^2)
				ENDFOR
			 ENDFOR
			 ...
			 sect_bar_min     = 0.0
			 sect_bar_max     = 10.0
			 fill_sect_table  = HCL_COLOR_TABLE(11, HUE_RANGE = [100.0, 300.0])
			 fill_sect_levels = FINDGEN(11)													;Set reflectivity contour levels
			 sect_bar_title   = 'Richardson Number'		 
			END		 

		6 : BEGIN	;Pressure difference between parent and nested domain (hardcoded for 30 May 2012 hi-res simulation)
	...
	IF st EQ 0 THEN BEGIN
		fill_sect1   = fill_sect
		cont_sect11  = cont_sect1
		cont_sect_o1 = cont_sect
		trop_sect1   = trop_sect
	ENDIF

	IF st EQ 1 THEN BEGIN
		fill_sect0   = fill_sect
		cont_sect10  = cont_sect1
		cont_sect_o0 = cont_sect
		trop_sect0   = trop_sect
	ENDIF

	st=st+1
ENDFOREACH

fill_sect = fill_sect1-fill_sect0

outdir  = !WRF_DIRECTORY + experiment + '/paper/plots/xsect_final/' 
epsfile = outdir + date_string + '_difference.eps'											
pdffile = outdir + date_string + '_difference.pdf'											
pngfile = outdir + date_string + '_difference.png'											
...
IF (map_type EQ 4) THEN BEGIN
	...
	tabler   = COLOR_24('red')												;Set reflectivity color table
...
IF (~KEYWORD_SET(pottemp)) THEN BEGIN
    ;CONTOUR,  cont_sect, FINDGEN(nsect), 0.001*zsect, $														;Overplot potential temperature contour cross-section
    ;	OVERPLOT = 1, $
    ;	FOLLOW	 = 1, $
	...
	IF (~(section_type EQ 0) OR (section_type EQ 1)) THEN BEGIN
        CONTOUR, cont_sect10, FINDGEN(nsect), 0.001*zsect, $														;Overplot cloud contour cross-section
        	OVERPLOT    = 1, $
        ...
        CONTOUR, cont_sect11, FINDGEN(nsect), 0.001*zsect, $														;Overplot cloud contour cross-section
        	OVERPLOT = 1, $
        	LEVELS   = cont_sect1_levels, $
        	C_COLOR  = cont_sect1_color, $
        	C_THICK  = cont_sect1_thick*thick_scale

		CONTOUR, cont_sect_o0, FINDGEN(nsect),  0.001*zsect, $															;Overplot O3 contour cross-section
			OVERPLOT    = 1, $
		...
		CONTOUR, cont_sect_o1, FINDGEN(nsect),  0.001*zsect, $															;Overplot O3 contour cross-section
			OVERPLOT = 1, $
			LEVELS   = [100.0], $;, 125.0, 150.0], $;[100.0, 150.0, 200.0, 250.0], $
			C_COLOR  = cont_sect_color, $
			C_THICK  = cont_sect_thick*thick_scale*3.0
...
IF (~KEYWORD_SET(pottemp)) THEN BEGIN
    OPLOT, 10*FINDGEN(nsect/10), 0.001*trop_sect1[0:*:10], $													;Draw tropopause altitudes as circles
    	PSYM = 8, SYMSIZE = 2.5 - KEYWORD_SET(eps)
    OPLOT, 10*FINDGEN(nsect/10), 0.001*trop_sect0[0:*:10], $													;Draw tropopause altitudes as circles
    	PSYM = 7, SYMSIZE = 2.5 - KEYWORD_SET(eps)
ENDIF ELSE BEGIN
