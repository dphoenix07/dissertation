PRO PLOT_OVERSHOOT_RELATIONSHIPS, event, scheme, start_date, end_date, $
	DOMAIN   = domain, $
	H2O_AVE	 = h2o_ave, $
	H2O_KG   = h2o_kg, $
	H2O_CO   = h2o_co, $
...
indir  = !WRF_DIRECTORY + event + '/' + scheme + '/reduced/overshoot_statistics_v2/'															;Set input directory
infile = FILE_SEARCH(indir + '*.nc', COUNT = nfile)							;Set input filepath

epsfile = outdir + 'updraft.eps'						;EPS filename
pdffile = outdir + 'updraft.pdf'						;PDF filename
pngfile = outdir + 'updraft.png'						;PNG filename
...
FOREACH date, date_arr DO BEGIN
	...
	IF (KEYWORD_SET(H2O_AVE)) THEN BEGIN
		overshoot_depth     = (WRF_READ_VAR('Max_Overshoot'     , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
		max_h2o 	        = (WRF_READ_VAR('Max_H2O'           , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values							;Read temperature variable from WRF output
		mean_h2o 	        = (WRF_READ_VAR('Mean_H2O'          , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values					
		mean_h2o_nc 	    = (WRF_READ_VAR('Mean_H2O_NC'       , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values					
		mean_ztrop		 	= (WRF_READ_VAR('Mean_Ztrop'		, date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
		mean_temp           = (WRF_READ_VAR('Mean_T'            , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
		cyl_temp            = (WRF_READ_VAR('Mean_Cyl_Temp'	    , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
		mean_o3             = (WRF_READ_VAR('Mean_O3'	        , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values	
		mean_co             = (WRF_READ_VAR('Mean_CO'	        , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values	
	...
	ENDIF ELSE IF (KEYWORD_SET(H2O_KG)) THEN BEGIN
		overshoot_depth     = (WRF_READ_VAR('Max_Overshoot'     , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
		mean_ztrop		 	= (WRF_READ_VAR('Mean_Ztrop'		, date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
		h2o_kg_strat		= (WRF_READ_VAR('H2O_NC_Strat_Mass' , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values	 
		mean_temp           = (WRF_READ_VAR('Mean_T'            , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
		cyl_temp            = (WRF_READ_VAR('Mean_Cyl_Temp'	    , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
...
;		ave_cyl_temp		  = [ave_cyl_temp		   , cyl_temp			]
	...
	ENDIF ELSE IF (KEYWORD_SET(CO)) THEN BEGIN
		max_co 	            = (WRF_READ_VAR('Max_CO'            , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
		mean_co             = (WRF_READ_VAR('Mean_CO'           , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
		mean_ztrop		 	= (WRF_READ_VAR('Mean_Ztrop'		, date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
		overshoot_depth     = (WRF_READ_VAR('Max_Overshoot'     , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
		mean_temp           = (WRF_READ_VAR('Mean_T'            , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
	...
	ENDIF ELSE IF (KEYWORD_SET(ICE)) THEN BEGIN
		mean_ice 	        = (WRF_READ_VAR('Mean_Ice'          , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values					
		overshoot_depth     = (WRF_READ_VAR('Max_Overshoot'     , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
		mean_ztrop		 	= (WRF_READ_VAR('Mean_Ztrop'		, date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
		mean_h2o 	        = (WRF_READ_VAR('Mean_H2O'          , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values					
		mean_h2o_nc 	    = (WRF_READ_VAR('Mean_H2O_NC'       , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values					
		mean_temp           = (WRF_READ_VAR('Mean_T'            , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
		mean_co             = (WRF_READ_VAR('Mean_CO'           , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
		unstable_layers_tot = (WRF_READ_VAR('Unstable_Layers'   , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values 
		cyl_temp            = (WRF_READ_VAR('Mean_Cyl_Temp'	    , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
	...
	ENDIF ELSE IF (KEYWORD_SET(UPDRAFT)) THEN BEGIN
		...
		mean_temp           = (WRF_READ_VAR('Mean_T'            , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
		mean_co             = (WRF_READ_VAR('Mean_CO'           , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
		updraft_size		= (WRF_READ_VAR('Updraft_Size'		, date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
		cyl_temp            = (WRF_READ_VAR('Mean_Cyl_Temp'	    , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
		mean_o3             = (WRF_READ_VAR('Mean_O3'	        , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values	
	...
	ENDIF ELSE IF (KEYWORD_SET(UNSTABLE)) THEN BEGIN
		...
		overshoot_depth     = (WRF_READ_VAR('Max_Overshoot'     , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
		unstable_layers_tot = (WRF_READ_VAR('Unstable_Layers'   , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values 
		unstable_layers_ic  = (WRF_READ_VAR('Unstable_Layers_IC', date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values 
		mean_temp           = (WRF_READ_VAR('Mean_T'            , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
		mean_h2o 	        = (WRF_READ_VAR('Mean_H2O'          , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values					
		mean_h2o_nc 	    = (WRF_READ_VAR('Mean_H2O_NC'       , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values					
		cyl_temp            = (WRF_READ_VAR('Mean_Cyl_Temp'	    , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
	...
	ENDIF ELSE IF (KEYWORD_SET(RHI)) THEN BEGIN
		...
		mean_h2o 	        = (WRF_READ_VAR('Mean_H2O'          , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values					
		mean_h2o_nc 	    = (WRF_READ_VAR('Mean_H2O_NC'       , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values					
		max_rhi		        = (WRF_READ_VAR('Max_RHI'           , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values					
		ave_rhi   		    = (WRF_READ_VAR('Mean_RHI'          , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
		cyl_temp            = (WRF_READ_VAR('Mean_Cyl_Temp'	    , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
		...
		mean_ztrop_total	  = [mean_ztrop_total	   , mean_ztrop			]
	ENDIF ELSE BEGIN	
		max_h2o_nc 	        = (WRF_READ_VAR('Max_H2O_NC'        , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values							;Read temperature variable from WRF output
		mean_h2o_nc 	    = (WRF_READ_VAR('Mean_H2O_NC'       , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values					
		max_perc_h2o 	    = (WRF_READ_VAR('Max_Perc_H2O'      , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values							;Read temperature variable from WRF output
		mean_perc_h2o 	    = (WRF_READ_VAR('Mean_Perc_H2O'     , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values					
		max_perc_h2o_nc 	= (WRF_READ_VAR('Max_Perc_H2O_NC'   , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values							;Read temperature variable from WRF output
		mean_perc_h2o_nc 	= (WRF_READ_VAR('Mean_Perc_H2O_NC'  , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values					
		max_cloud 	        = (WRF_READ_VAR('Max_Cloud'         , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values							;Read temperature variable from WRF output
		mean_cloud 	        = (WRF_READ_VAR('Mean_Cloud'        , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values					
		total_cloud 	    = (WRF_READ_VAR('Total_Cloud_Mass'  , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values					
		max_ice 	        = (WRF_READ_VAR('Max_Ice'           , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values							;Read temperature variable from WRF output
	;	max_rhi		        = (WRF_READ_VAR('Max_RHI'           , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values					
	;	ave_rhi   		    = (WRF_READ_VAR('Mean_RHI'          , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
		min_o3 	            = (WRF_READ_VAR('Min_O3'	        , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
		mean_o3             = (WRF_READ_VAR('Mean_O3'	        , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values	
		max_w	            = (WRF_READ_VAR('Max_W'             , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values

		mean_trop_refl  = (WRF_READ_VAR('Mean_Refl'            , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
		mean_trop_cloud = (WRF_READ_VAR('Mean_Trop_Cloud'      , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
		trop_cloud_kg   = (WRF_READ_VAR('Total_Trop_Cloud_Mass', date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
		cyl_temp	    = (WRF_READ_VAR('Cylinder_Temps'	   , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
		cyl_u		    = (WRF_READ_VAR('Cylinder_U'		   , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values	
		cyl_v		    = (WRF_READ_VAR('Cylinder_V'		   , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
		cyl_z		    = (WRF_READ_VAR('Cylinder_Z'		   , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
		cyl_dh2o	    = (WRF_READ_VAR('Cylinder_dh2o_dz'	   , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values 
		flow_u_5km	    = (WRF_READ_VAR('Mean_U_5km'		   , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values 
		flow_V_5km	    = (WRF_READ_VAR('Mean_V_5km'		   , date, event, scheme, DOMAIN=domain, INDICES=region, OVER=1)).values
...
;
	...
	PLOT, overshoot_total, ave_cyl_temp, /NODATA, $
				xtitle = 'Overshoot Depth (km)', ytitle = "Mean Cylinder Temp (K)", PSYM = 4, $
;				yrange = [240, 190], ylog = 0, xrange = [0,5], title = start_date, POSITION = map_pos
				yrange = [225, 210], ylog = 0, xrange = [0,5], title = start_date, POSITION = map_pos
...
;
		...
		IF (event EQ '20130517') THEN BEGIN
			table = HCL_COLOR_TABLE(15, HUE_RANGE = [100.0, 300.0])
			h2olevels = FINDGEN(15)+48.0
			ih2o = WHERE(h2olevels EQ ROUND(10.0*ave_h2o_nc_total[s]))
		ENDIF

		IF (event EQ '20120530_ncar') THEN BEGIN
			table = HCL_COLOR_TABLE(65, HUE_RANGE = [100.0, 300.0])
			h2olevels = FINDGEN(65)
			;table = HCL_COLOR_TABLE(20, HUE_RANGE = [100.0, 300.0])
			;h2olevels = FINDGEN(20)
			ih2o = WHERE(h2olevels EQ ROUND(ave_h2o_nc_total[s]))
		ENDIF
...
;
	...
	;	RANGE = [0, 250], $
	;	TICKS = 5, $
	;	TITLE = 'Max H2O3 Concentration (ppm)', $
	;	POSIT = bar_pos
ENDIF
...
IF (KEYWORD_SET(RHI)) THEN BEGIN
	...
	PLOT, ave_rhi_total, ave_cyl_temp, /NODATA, $
				...
				yrange = [240, 190], ylog = 0, xrange = [0,10], title = start_date, POSITION = map_pos
;				yrange = [225, 210], ylog = 0, xrange = [1.5,3.5], title = start_date, POSITION = map_pos
...
;				yrange = [240, 190], ylog = 0, xrange = [1,600], title = start_date, POSITION = map_pos
				yrange = [230, 180], ylog = 0, xrange = [1,200], title = start_date, POSITION = map_pos
;				yrange = [225, 210], ylog = 0, xrange = [1,10000], title = start_date, POSITION = map_pos
...
;;	TITLE = 'Normalized H2O (NC) Concentration (x100)', $
	...
	;	RANGE = [25, 75], $
	;	TICKS = 5, $
	;	TITLE = 'Average H2O Concentration (ppbv)', $
	;	POSIT = bar_pos
...
;				yrange = [240, 190], ylog = 0, xrange = [0,50], title = start_date, POSITION = map_pos
				yrange = [225, 210], ylog = 0, xrange = [0,50], title = start_date, POSITION = map_pos
