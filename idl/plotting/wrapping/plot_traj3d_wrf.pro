...
indir  = '/data3/dphoenix/wrf/20120530_ncar/d03_30km_icloud/traj3d/20120530_trailwrap.ncd'															;Set input directory
...
;;+Spaghetti plots of altitude change from cloud air
;zone = WHERE((r.x[*,0] GT 262.24579) AND (r.x[*,0] LT 263.57642) AND (r.y[*,0] GT 37.2695) $
;		AND (r.y[*,0] LT 37.7827))
...
;;+Spaghetti plots of altitude change from small updraft region
;zone = WHERE((r.x[*,0] GT 258.85101) AND (r.x[*,0] LT 259.53299) AND (r.y[*,0] GT 39.4594) $
;		AND (r.y[*,0] LT 39.6706))

date = MAKE_DATE(2012,5,30,20,30)
...
icloud = WHERE(cloud GT 1.0E-5, count, COMPLEMENT=iclear)
xclear = x[iclear] + 360.0
yclear = y[iclear]
zclear = z[iclear]
clear_zone = WHERE((r.x[zone,0] GT MIN(xclear) AND (r.x[zone,0] LT MAX(xclear)) AND $
			(r.y[zone,0] GT MIN(yclear)) AND (r.y[zone,0] LT MAX(yclear)) AND $
			(r.z[zone,0] GT MIN(zclear)) AND (r.z[zone,0] LT MAX(zclear))), count)

zone1 = zone[clear_zone]
alt   = WHERE((r.z[zone1,0] GE 7000.0) AND (r.z[zone1,0] LE 10000.0))
alt1  = alt[0:N_ELEMENTS(alt)-1:100] 

PLOT, r.z[0,*], COLOR = COLOR_24('black'), YRANGE = [4000.0, 14000.0], /NODATA

dz  = 200.0
nz  = MAX(r.z[zone1,0],/NAN)
dim = SIZE(r.z, /DIMENSIONS)
ci  = 0
colors = HCL_COLOR_TABLE(N_ELEMENTS(FINDGEN((13000.0-8000.0)/dz)), HUE_RANGE = [300.0, 400.0])								;Set color table

FOR ii = 10400.0, 13000.0, dz DO BEGIN
	med   = [ ]
	date = MAKE_DATE(2012,5,30,20,30)
	FOR tt = 0, dim[1]-4 DO BEGIN
		dt = 300
		PRINT, date
		x  	   = (WRF_READ_VAR('Longitude',       date, run, experiment, DOMAIN = domain, INDICES = region)).values			;Read variables
		y      = (WRF_READ_VAR('Latitude',        date, run, experiment, DOMAIN = domain, INDICES = region)).values	
		...
		yclear = y[iclear]
		zclear = z[iclear]
		clear_zone = WHERE((r.x[zone,tt] GT MIN(xclear) AND (r.x[zone,tt] LT MAX(xclear)) AND $
		(r.y[zone,tt] GT MIN(yclear)) AND (r.y[zone,tt] LT MAX(yclear)) AND $
		(r.z[zone,tt] GT MIN(zclear)) AND (r.z[zone,tt] LT MAX(zclear))), count)

		zone1 = zone[clear_zone]

		lev = WHERE(r.z[zone1,0] EQ ii, count)
		med   = [med, MEDIAN(r.z[zone1[lev],tt])]

	    date = TIME_INC(date, dt)
	ENDFOR
	ci = ci + 1
	OPLOT, med, COLOR = colors[ci], THICK=3
ENDFOR
STOP
...
FOR ss = 0, 3 DO BEGIN
	...
	indir  = '/data3/dphoenix/wrf/20120530_ncar/d03_30km_icloud/traj3d/20120530_cloud.ncd'															;Set input directory
	r = TRAJ3D_READ_FILE_3(indir)
...
;	hgt_pos = hgt_pos[0:num-1:int]
    ...
    outdir  = !WRF_DIRECTORY + run + '/' + experiment + '/plots/trajectory_timeseries/'
