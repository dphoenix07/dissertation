PRO OZONE_MASS_LAYER_V2_NOMASS, run, experiment, start_date, end_date, $
	DOMAIN        = domain, $
	REGION        = region, $
	WRAPPING      = wrapping, $
...
FOREACH date, date_arr DO BEGIN
	...
	o3_ugm3_conv  = FLTARR(dim[0],dim[1],dim[2])*!Values.F_NaN
	o3_ugm3_nconv = FLTARR(dim[0],dim[1],dim[2])*!Values.F_NaN

	conv_count=0
	nconv_count=0
	iconv = WHERE(updrt GT 0.1, conv_count, COMPLEMENT=nconv)
	o3_ugm3_conv [iconv] = o3_ugm3[iconv]
	o3_ugm3_conv [nconv] = !Values.F_NaN
	o3_ugm3_nconv[nconv] = o3_ugm3[nconv]
	o3_ugm3_nconv[iconv] = !Values.F_NaN
		
	FOR ii = 5, dim[0]-6 DO BEGIN
		FOR jj = 5, dim[1]-6 DO BEGIN
			index2= MEAN(VALUE_LOCATE(REFORM(z[ii,jj,*],dim[2],1,1),REFORM(trop_3d[ii,jj,*],dim[2],1,1)))
			index1 = index2-8
	    	...
	    	FOR kk = index1, index2 DO BEGIN
				...
				o3_kg_conv[kk]   = o3_ugm3_conv[ii,jj,kk]*dx*dx*(z[ii,jj,kk+1]-z[ii,jj,kk])*1.0E-9 
				o3_kg_nconv[kk]  = o3_ugm3_nconv[ii,jj,kk]*dx*dx*(z[ii,jj,kk+1]-z[ii,jj,kk])*1.0E-9 
			ENDFOR
			...
			o3_perc_2d_copy4[ii,jj] = MEAN((o3_kg[index1:index2]),/NAN)
			o3_perc_2d_copy3[ii,jj] = TOTAL((o3_kg[index1:index2]),/NAN)
			o3_perc_2d_conv [ii,jj] = MEAN((o3_kg_conv[index1:index2]),/NAN)
			o3_perc_2d_nconv[ii,jj] = MEAN((o3_kg_nconv[index1:index2]),/NAN)
...
;Do (O3 mass/Air mass)*wind for flux and then divide by dx--> (%kg/s)		
	...
	value_count = WHERE(flux_o3_left NE 0.0, count)
	flux_ew1 = MEAN(((flux_o3_left) - (flux_o3_right)), /NAN); / ((dim[0]-11) * dx)
	flux_ns1 = MEAN(((flux_o3_bottom) - (flux_o3_top)), /NAN); / ((dim[1]-11) * dx)
	flux_ew = TOTAL(((flux_o3_left) - (flux_o3_right)), /NAN); / ((dim[0]-11) * dx)
	flux_ns = TOTAL(((flux_o3_bottom) - (flux_o3_top)), /NAN); / ((dim[1]-11) * dx)
	flux_ew2 = TOTAL(((flux_o3_left) - (flux_o3_right)), /NAN) / ((dim[0]-11) * dx)
	flux_ns2 = TOTAL(((flux_o3_bottom) - (flux_o3_top)), /NAN) / ((dim[1]-11) * dx)
	flux_ew3 = MEAN(((flux_o3_left) - (flux_o3_right)), /NAN) / ((dim[0]-11) * dx)
	flux_ns3 = MEAN(((flux_o3_bottom) - (flux_o3_top)), /NAN) / ((dim[1]-11) * dx)

	flux_ew4 = (TOTAL(((flux_o3_left) - (flux_o3_right)), /NAN)/count) / ((dim[0]-11) * dx)
	flux_ns4 = (TOTAL(((flux_o3_bottom) - (flux_o3_top)), /NAN)/count) / ((dim[1]-11) * dx)

	flux_tot = flux_ew4 + flux_ns4
	time_flux = [[[time_flux]],[[flux_tot]]]
  		
    ozone_array   = [ozone_array, MEAN(o3_perc_2d_copy4,/NAN)]
    ;ozone_array_total  = [ozone_array, TOTAL(o3_perc_2d_copy3,/NAN)]
    ozone_array_conv   = [ozone_array_conv, MEAN(o3_perc_2d_conv,/NAN)]
    ozone_array_nconv  = [ozone_array_nconv, MEAN(o3_perc_2d_nconv,/NAN)]

    ;ozone_array1   = [ozone_array1, TOTAL(o3_perc_2d_copy4,/NAN)/TOTAL(FINITE(o3_perc_2d_copy4))]
    ;ozone_array_conv1   = [ozone_array_conv1, TOTAL(o3_perc_2d_conv,/NAN)/TOTAL(FINITE(ozone_array_conv))]
    ;ozone_array_nconv1   = [ozone_array_nconv1, TOTAL(o3_perc_2d_nconv,/NAN)/TOTAL(FINITE(o3_perc_2d_nconv))]
...
epsfile = outdir + dom_string + '_' + date_string + '_kg_v2_nomass.eps'											;EPS filename
...
ozone_vertflux_conv =  ozone_array_conv
...
IF ((run EQ '20120530_ncar') AND (experiment EQ 'd02_30km')) THEN BEGIN 
    PLOT, SHIFT(do3_dt,1), $
    	...
    	TITLE    = 'Ozone % Mass of Layer: 2.5km, 22Z-05Z', $
    	CHARSIZE = 2, $
    	/NODATA
    ...
    OPLOT, ozone_vertflux, COLOR = COLOR_24('gray50'), THICK=3
    ...
    OPLOT, (ozone_vertflux_conv+ozone_vertflux_nconv)/2, COLOR = COLOR_24('gray10'), THICK=3, LINESTYLE=2

    
    XYOUTS, 7.0, 190.0, 'dO3/dt' , COLOR = COLOR_24('red'      ), CHARSIZE=2, /DATA
    XYOUTS, 7.0, 185.0, 'O3 flux', COLOR = COLOR_24('blue'     ), CHARSIZE=2, /DATA
    XYOUTS, 7.0, 180.0, 'Net'    , COLOR = COLOR_24('darkgreen'), CHARSIZE=2, /DATA
    XYOUTS, 7.0, 175.0, 'o3'     , COLOR = COLOR_24('gray50'   ), CHARSIZE=2, /DATA
    XYOUTS, 7.0, 170.0, 'o3-conv' , COLOR = COLOR_24('gray30'   ), CHARSIZE=2, /DATA
    XYOUTS, 7.0, 165.0, 'o3-nconv', COLOR = COLOR_24('gray70'   ), CHARSIZE=2, /DATA
ENDIF
...
IF ((run EQ '20120530_ncar') AND (experiment EQ 'd03_30km')) THEN BEGIN 
    ...
    XYOUTS, 72, 7.2, 'dO3/dt' , COLOR = COLOR_24('red'      ), CHARSIZE=2, /DATA
    XYOUTS, 72, 7.0, 'O3 flux', COLOR = COLOR_24('blue'     ), CHARSIZE=2, /DATA
    XYOUTS, 72, 6.8, 'Net'    , COLOR = COLOR_24('darkgreen'), CHARSIZE=2, /DATA
    XYOUTS, 72, 6.6, 'o3'     , COLOR = COLOR_24('gray50'   ), CHARSIZE=2, /DATA
    XYOUTS, 72, 6.4, 'o3-conv' , COLOR = COLOR_24('gray30'   ), CHARSIZE=2, /DATA
    XYOUTS, 72, 6.2, 'o3-nconv', COLOR = COLOR_24('gray70'   ), CHARSIZE=2, /DATA
ENDIF
