...
FOREACH date, date_arr DO BEGIN
	...
	air_gm3_conv  = FLTARR(dim[0],dim[1],dim[2])*!Values.F_NaN
	air_gm3_nconv = FLTARR(dim[0],dim[1],dim[2])*!Values.F_NaN
	...
	air_gm3_conv [iconv] =	air_gm3	[iconv]
	air_gm3_conv [nconv] =	!Values.F_NaN
	air_gm3_nconv[nconv] =	air_gm3 [nconv]	
	air_gm3_nconv[iconv] =  !Values.F_NaN
	...
	FOR ii = 5, dim[0]-6 DO BEGIN
		FOR jj = 5, dim[1]-6 DO BEGIN
	    	...
	    	FOR kk = index1, index2 DO BEGIN
				...
				tot_mass_conv[kk]= air_gm3_conv[ii,jj,kk]*dx*dx*(z[ii,jj,kk+1]-z[ii,jj,kk])*1.0E-3	
				o3_kg_nconv[kk]   = o3_ugm3_nconv[ii,jj,kk]*dx*dx*(z[ii,jj,kk+1]-z[ii,jj,kk])*1.0E-9 
				tot_mass_nconv[kk]= air_gm3_nconv[ii,jj,kk]*dx*dx*(z[ii,jj,kk+1]-z[ii,jj,kk])*1.0E-3	
			...
			o3_perc_2d_copy4[ii,jj] = MEAN((o3_kg[index1:index2]/total_mass[index1:index2]),/NAN)
			o3_perc_2d_conv [ii,jj] = MEAN((o3_kg_conv[index1:index2]/tot_mass_conv[index1:index2]),/NAN)
			o3_perc_2d_nconv[ii,jj] = MEAN((o3_kg_nconv[index1:index2]/tot_mass_nconv[index1:index2]),/NAN)
...
;Do (O3 mass/Air mass)*wind for flux and then divide by dx--> (%kg/s)		
	...
	flux_ew4 = (TOTAL(((flux_o3_left/flux_air_left) - (flux_o3_right/flux_air_right)), /NAN)/count) / ((dim[0]-11) * dx)
	flux_ns4 = (TOTAL(((flux_o3_bottom/flux_air_bottom) - (flux_o3_top/flux_air_top)), /NAN)/count) / ((dim[1]-11) * dx)
...
epsfile = outdir + dom_string + '_' + date_string + '_%kg_new2.eps'											;EPS filename
