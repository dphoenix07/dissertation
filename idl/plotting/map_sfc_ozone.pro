PRO MAP_SFC_OZONE, date, run, experiment, $
	LEV          = lev, $
	PLT_NOX      = plt_nox, $
	PLT_CO		 = plt_co, $
	OFFSET       = offset, $
...
;nox  = (WRF_READ_VAR('NO' 	   , date, run, experiment, DOMAIN = domain, INDICES = region)).values * 1.0E6
;nox += (WRF_READ_VAR('NO2' 	   , date, run, experiment, DOMAIN = domain, INDICES = region)).values * 1.0E6
;co   = (WRF_READ_VAR('CO'	   , date, run, experiment, DOMAIN = domain, INDICES = region)).values * 1.0E3


PRINT, "mean tropopause = ", MEAN(ztrop, /NAN)
FOR kk = (lev), lev DO BEGIN
PRINT, kk, MEAN(z[*,*,kk],/NAN)
;mean_o = MEDIAN(o[*,*,kk], 33)
;anom_o = o[*,*,kk] - mean_o

;PRINT, MAX(anom_o,/NAN)
...
ENDIF ELSE IF KEYWORD_SET(plt_nox) THEN BEGIN
	...
	key 	= 'sfc_nox'
ENDIF ELSE BEGIN
	table   = HCL_COLOR_TABLE(30, HUE = [150.0, 360.0])																	;Set color table
	rlevels = 5.0 * FINDGEN(N_ELEMENTS(table)) 															;Set reflectivity contour levels
	key     = 'sfc_o3'
ENDELSE
...
MAP_SET, yc, xc, 0, CONIC = 1, /NOBORDER, $																;Draw map
	...
	TITLE 	  = 'Mean Trop = ' + STRMID(STRING(MEAN(ztrop, /NAN)),5) + 'm Level = ' + $
					STRMID(STRING(MEAN(z[*,*,kk],/NAN)),5) + 'm valid ' + date_string, $
	POSITION  = map_pos
...
ENDIF ELSE $	
	...
	IF KEYWORD_SET(plt_co) THEN BEGIN
		CONTOUR, co[*,*,kk], x.values, y.values, $											;Surface nox
			OVERPLOT  = 1, $
			FILL      = 1, $
			LEVELS    = rlevels, $
			C_COLOR   = table
	ENDIF ELSE IF KEYWORD_SET(plt_nox) THEN BEGIN
		CONTOUR, nox[*,*,kk], x.values, y.values, $											;Surface nox
			OVERPLOT  = 1, $
			FILL      = 1, $
			LEVELS    = rlevels, $
			C_COLOR   = table
	ENDIF ELSE BEGIN
		CONTOUR, o[*,*,kk], x.values, y.values, $													;Surface ozone
			OVERPLOT  = 1, $
			FILL      = 1, $
			LEVELS    = rlevels, $
			C_COLOR   = table
...
ureg = TRIGRID(x.values, y.values, u[*,*,kk], tri, XOUT = xreg, YOUT = yreg)
vreg = TRIGRID(x.values, y.values, v[*,*,kk], tri, XOUT = xreg, YOUT = yreg)
