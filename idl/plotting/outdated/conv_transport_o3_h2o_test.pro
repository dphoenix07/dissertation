PRO CONV_TRANSPORT_O3_H2O_TEST, event, scheme, start_date, end_date, $
	DOMAIN     = domain, $
	BINNED     = binned, $
...
;    	j = 0
;      	FOR j = 0, 3 DO BEGIN
;      		IF (j EQ 0) THEN region = [0		 ,0			,((dim[0]/2)-1),((dim[1]/2)-1)]
;      	 	IF (j EQ 1) THEN region = [(dim[0]/2),0			,  dim[0]      ,((dim[1]/2)-1)]
;     	  	IF (j EQ 2) THEN region = [0		 ,(dim[1]/2),((dim[0]/2)-1),  dim[1]      ]
;      	 	IF (j EQ 3) THEN region = [(dim[0]/2),(dim[1]/2),  dim[0]	   ,  dim[1]      ]
;     	  	PRINT, region	
