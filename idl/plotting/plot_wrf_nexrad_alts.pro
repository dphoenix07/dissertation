...
IF KEYWORD_SET(wrapping) THEN BEGIN
    ...
    epsfile = !WRF_DIRECTORY + event + '/paper/plots/histograms/wrf_storm_alt.eps'
	pngfile = !WRF_DIRECTORY + event + '/paper/plots/histograms/wrf_storm_alt.png'
	FILE_MKDIR, !WRF_DIRECTORY + event + '/paper/plots/histograms/'
ENDIF	
...
;	FOR dom = 2 DO BEGIN
		...
		FOR s = 0, nt DO BEGIN
			...
			Zwrf_cld = MAX((cloud.values GT (1.0E-5))*(Z.values), DIM = 3)																	;Get WRF cloud maximum altitudes
...
;	OPLOT, zbin, 100.0*(FLOAT(hist_wrf[*,2])/TOTAL(hist_wrf[*,2])), THICK = 2, PSYM = 10, COLOR = color[2]
	...
	PLOT, zbin, zbin, /NODATA, $
		XRANGE = [6,16],$;[z0 + 0.5*dz, z1 - 0.5*dz], $
		XSTYLE = 1, $
		XTITLE = 'Storm Altitude (km)', $
		YRANGE = [0, 85], $
		YSTYLE = 1, $
